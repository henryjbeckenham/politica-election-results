...
from politica_erd.install_2019_release import ELECTION_2019
...
class Stage143HistoricalIngestionTests(unittest.TestCase):
    def test_all_45_official_aec_sources_are_present_and_checksum_pinned(self):
        manifest = json.loads(
            (PROJECT_ROOT / "data/manifests/aec_2019_sources.json").read_text(
                encoding="utf-8"
            )
        )
        checksum_contract = yaml.safe_load(
            (PROJECT_ROOT / "config/source_checksums_2019.yml").read_text(
                encoding="utf-8"
            )
        )["sha256_by_source_key"]
        self.assertEqual(manifest["election_id"], ELECTION_2019)
        self.assertEqual(manifest["event_id"], "24310")
        ...
        for row in manifest["sources"]:
            ...
            self.assertTrue(row["url"].startswith("https://results.aec.gov.au/24310/"))
    ...
    def test_prevalidated_delta_contains_the_complete_relational_import(self):
        ...
        expected_counts = {
            "core.contest": 159,
            "core.candidacy": 1514,
            "geography.election_reporting_unit": 9034,
            "results.vote_result": 242038,
            "results.participation_result": 1908,
            "results.contest_outcome": 191,
            "count.count_round": 2562,
            "count.count_candidate_total": 135171,
            "count.preference_transfer": 19698,
            "ballot.ballot_dataset": 8,
            "provenance.source_file_revision": 45,
            "provenance.row_lineage": 414930,
        }
    ...
    def test_import_report_reconciles_complete_house_senate_and_ballot_data(self):
        report = json.loads(
            (PROJECT_ROOT / "dist/stage_14_3_2019_import_report.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        self.assertEqual(report["table_counts"]["candidacies"], 1514)
        self.assertEqual(report["table_counts"]["formal_ballots"], 14_604_925)
        self.assertEqual(
            report["table_counts"]["formal_ballot_preferences"], 98_547_026
        )

    def test_2019_map_contract_uses_the_151_official_aec_divisions(self):
        contract = yaml.safe_load(
            (PROJECT_ROOT / "config/electorate_boundaries_2019.yml").read_text(
                encoding="utf-8"
            )
        )
        geometry_path = PROJECT_ROOT / contract["derived_geometry"]["source_path"]
        geometry = json.loads(geometry_path.read_text(encoding="utf-8"))
        self.assertEqual(contract["election_id"], ELECTION_2019)
    ...
    def test_batch_2019_adapter_does_not_shadow_interactive_import_detection(self):
        config = yaml.safe_load(
            (PROJECT_ROOT / "config/adapters/aec_2019.yml").read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(config["catalogue_visibility"], "batch_only")
        self.assertEqual(config["election"]["election_id"], ELECTION_2019)
