...
from politica_erd.install_2016_release import ELECTION_2016
...
class Stage144HistoricalIngestionTests(unittest.TestCase):
    def test_all_46_official_aec_sources_are_present_and_checksum_pinned(self):
        manifest = json.loads(
        ...
        checksum_contract = yaml.safe_load(
            (PROJECT_ROOT / "config/source_checksums_2016.yml").read_text(
                encoding="utf-8"
            )
        )["sha256_by_source_key"]
        self.assertEqual(manifest["election_id"], ELECTION_2016)
        self.assertEqual(manifest["event_id"], "20499")
        self.assertEqual(manifest["source_count"], 46)
        ...
        for row in manifest["sources"]:
            ...
            self.assertTrue(row["url"].startswith("https://results.aec.gov.au/20499/"))
    ...
    def test_prevalidated_delta_contains_the_complete_relational_import(self):
        manifest_path = PROJECT_ROOT / "data/manifests/aec_2016_delta_tables.json"
        ...
        expected_counts = {
            "core.contest": 158,
            "core.candidacy": 1625,
            "geography.election_reporting_unit": 8486,
            "results.vote_result": 268558,
            "results.participation_result": 1896,
            "results.contest_outcome": 226,
            "count.count_round": 4956,
            "count.count_candidate_total": 458285,
            "count.preference_transfer": 31048,
            "ballot.ballot_dataset": 8,
            "provenance.source_file_revision": 46,
            "provenance.row_lineage": 778195,
        }
    ...
    def test_import_report_reconciles_complete_house_senate_and_ballot_data(self):
        report = json.loads(
            (PROJECT_ROOT / "dist/stage_14_4_2016_import_report.json").read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(report["status"], "PASS")
        self.assertEqual(report["validation"]["blocker_count"], 0)
        self.assertEqual(report["source_count"], 46)
        self.assertEqual(report["table_counts"]["contests"], 158)
        self.assertEqual(report["table_counts"]["candidacies"], 1625)
        self.assertEqual(report["table_counts"]["formal_ballots"], 13_838_900)
        self.assertEqual(
            report["table_counts"]["formal_ballot_preferences"], 93_842_251
        )

    def test_compact_2016_ballots_reconcile_atl_btl_and_preference_paths(self):
        report = json.loads(
            (PROJECT_ROOT / "dist/stage_14_4_2016_import_report.json").read_text(
                encoding="utf-8"
            )
        ...
        self.assertEqual(formal["ballot_count"], 13_838_900)
        self.assertEqual(formal["preference_count"], 93_842_251)
        self.assertEqual(formal["above_the_line_ballot_count"], 12_934_792)
        self.assertEqual(formal["below_the_line_ballot_count"], 904_108)
        self.assertEqual(formal["file_count"], 53)

    def test_publication_ceiling_covers_complete_double_dissolution_count(self):
        report = json.loads(
            (PROJECT_ROOT / "dist/stage_14_4_2016_import_report.json").read_text(
                encoding="utf-8"
            )
        ...
        self.assertEqual(senate_rows, 452_193)
        self.assertLessEqual(senate_rows, AppSettings().publication_max_rows)

    def test_2016_map_contract_uses_the_150_official_aec_divisions(self):
        contract = yaml.safe_load(
            (PROJECT_ROOT / "config/electorate_boundaries_2016.yml").read_text(
                encoding="utf-8"
            )
        )
        geometry_path = PROJECT_ROOT / contract["derived_geometry"]["source_path"]
        geometry = json.loads(geometry_path.read_text(encoding="utf-8"))
        self.assertEqual(contract["election_id"], ELECTION_2016)
    ...
    def test_frontend_registers_election_selection_and_election_scoped_assets(self):
        ...
        self.assertIn("election_fed_2016_07_02_general", results)
        self.assertIn("double-dissolution election", results)
        self.assertIn("/feeds/${election}${feedId}", dom)
        self.assertIn("/visualisations${suffix}", dom)

    def test_batch_2016_adapter_does_not_shadow_interactive_import_detection(self):
        config = yaml.safe_load(
            (PROJECT_ROOT / "config/adapters/aec_2016.yml").read_text(
                encoding="utf-8"
            )
        )
        self.assertEqual(config["catalogue_visibility"], "batch_only")
        self.assertEqual(config["election"]["election_id"], ELECTION_2016)
