...
on_exit() {
  ...
  if [ "$status" -ne 0 ]; then
    echo
    echo "Stage 9 installation stopped before every verification passed."
...
if [ ! -d "$PAYLOAD/src/politica_erd" ] || [ ! -f "$PAYLOAD/pyproject.toml" ]; then
  echo "The Stage 9 payload is missing. Extract the complete ZIP before running this file."
  exit 1
fi
...
if ! grep -Eq '^version = "0\.(8|9)\.0"$' "$TARGET/pyproject.toml"; then
  echo "Stage 9 expects the installed application to be version 0.8.0 or 0.9.0."
  echo "No change was made."
  exit 1
fi
...
for filename in \
  ...
  if [ ! -f "$TARGET/data/raw/aec/2025_federal/31496/final/$filename" ]; then
    echo "The required installed Stage 8 test source is missing:"
    echo "$TARGET/data/raw/aec/2025_federal/31496/final/$filename"
    echo "No change was made."
    exit 1
  fi
...
BACKUP="$BACKUP_ROOT/Stage8-before-0.9.0-$STAMP"
mkdir -p "$BACKUP"
...
echo "$BACKUP"
echo
echo "Installing Stage 9 code without changing data/app, active releases, raw election data or .env..."
...
mkdir -p "$TARGET/dist"
for report in stage_9_build_manifest.json stage_9_test_report.json stage_9_integration_report.json; do
...
echo
echo "Running Stage 4, Stage 5, Stage 6, Stage 7, Stage 8 and Stage 9 workflow tests..."
echo "The DOP and ballot test can be quiet for several minutes; wait until Terminal prints the final OK."
uv run python -m unittest -v tests.test_stage4_workflow tests.test_stage5_workflow tests.test_stage6_workflow tests.test_stage7_workflow tests.test_stage8_workflow tests.test_stage9_explorer
...
echo
echo "Stage 9 v0.9.0 is installed and verified."
