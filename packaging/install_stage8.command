...
on_exit() {
  ...
  if [ "$status" -ne 0 ]; then
    echo
    echo "Stage 8 installation stopped before every verification passed."
...
if [ ! -d "$PAYLOAD/src/politica_erd" ] || [ ! -f "$PAYLOAD/pyproject.toml" ]; then
  echo "The Stage 8 payload is missing. Extract the complete ZIP before running this file."
  exit 1
fi
...
if ! grep -Eq '^version = "0\.(7|8)\.0"$' "$TARGET/pyproject.toml"; then
  echo "Stage 8 expects the installed application to be version 0.7.0 or 0.8.0."
  echo "No change was made."
  exit 1
fi
...
BACKUP="$BACKUP_ROOT/Stage7-before-0.8.0-$STAMP"
mkdir -p "$BACKUP"
...
echo "$BACKUP"
echo
echo "Installing Stage 8 code without changing data/app, active releases, raw election data or .env..."
...
mkdir -p "$TARGET/dist"
for report in stage_8_build_manifest.json stage_8_test_report.json stage_8_integration_report.json; do
...
echo
echo "Running Stage 4, Stage 5, Stage 6, Stage 7 and Stage 8 workflow tests..."
echo "The DOP and ballot test can be quiet for several minutes; wait until Terminal prints the final OK."
uv run python -m unittest -v tests.test_stage4_workflow tests.test_stage5_workflow tests.test_stage6_workflow tests.test_stage7_workflow tests.test_stage8_workflow
...
echo
echo "Stage 8 v0.8.0 is installed and verified."
