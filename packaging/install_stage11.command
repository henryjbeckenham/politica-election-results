...
on_exit() {
  ...
  if [ "$status" -ne 0 ]; then
    echo
    echo "Stage 11 stopped before every verification passed."
...
for required in \
  ...
  if [ ! -e "$PAYLOAD/$required" ]; then
    echo "The Stage 11 payload is incomplete. Extract the complete ZIP before running this file."
...
if ! grep -Eq '^version = "1\.(0\.1|1\.0)"$' "$TARGET/pyproject.toml"; then
  echo "Stage 11 expects the installed application to be version 1.0.1 or 1.1.0."
  echo "No change was made."
  exit 1
fi
...
BACKUP="$BACKUP_ROOT/Stage10.1-before-1.1.0-$STAMP"
mkdir -p "$BACKUP"
...
echo "$BACKUP"
echo
echo "Installing Stage 11 code without changing data/app, active releases, raw election data or .env..."
...
mkdir -p "$TARGET/dist"
for report in stage_11_build_manifest.json stage_11_test_report.json stage_11_integration_report.json; do
...
echo
echo "Running the complete Stage 4–11 regression suite..."
...
if [ "$ACTIVE_BEFORE" != "$ACTIVE_AFTER" ]; then
  echo "The active release pointer changed unexpectedly; Stage 11 verification failed."
  exit 1
fi

echo
echo "Stage 11 v1.1.0 is installed and verified."
