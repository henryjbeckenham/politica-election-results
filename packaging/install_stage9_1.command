...
on_exit() {
  ...
  if [ "$status" -ne 0 ]; then
    echo
    echo "Stage 9.1 stopped before every verification passed."
    echo "The prior immutable release was not deleted or modified."
    echo "No raw election source, Google Sheets credential or Grand Database row was changed."
    if [ -n "$BACKUP" ]; then
...
if [ ! -d "$PAYLOAD/src/politica_erd" ] || [ ! -f "$PAYLOAD/pyproject.toml" ]; then
  echo "The Stage 9.1 payload is missing. Extract the complete ZIP before running this file."
  exit 1
fi
...
if ! grep -Eq '^version = "0\.9\.(0|1)"$' "$TARGET/pyproject.toml"; then
  echo "Stage 9.1 expects the installed application to be version 0.9.0 or 0.9.1."
  echo "No change was made."
  exit 1
fi
...
BACKUP="$BACKUP_ROOT/Stage9-before-0.9.1-$STAMP"
mkdir -p "$BACKUP"
...
echo "$BACKUP"
echo
echo "Installing Stage 9.1 code. Existing releases and raw sources are not being edited..."
...
mkdir -p "$TARGET/dist"
for report in stage_9_1_build_manifest.json stage_9_1_test_report.json stage_9_1_integration_report.json; do
...
echo
echo "Running the complete Stage 4–9.1 regression suite..."
...
uv run politica-erd-correct-tcp-measures
...
echo
echo "Stage 9.1 v0.9.1 is installed, corrected and verified."
echo "The active vote-result count remains 213,328; 34 historical TCP facts are retained as superseded."
echo "People, Parties, Constituencies, raw sources and Google Sheets configuration remain in place."
