...
on_exit() {
  ...
  if [ "$status" -ne 0 ]; then
    echo
    echo "Stage 10 stopped before every verification passed."
    echo "No active or historical release, raw source, Google Sheets credential or Grand Database row was changed."
    if [ -n "$BACKUP" ]; then
...
if [ ! -d "$PAYLOAD/src/politica_erd" ] || [ ! -f "$PAYLOAD/pyproject.toml" ]; then
  echo "The Stage 10 payload is missing. Extract the complete ZIP before running this file."
  exit 1
fi
...
if ! grep -Eq '^version = "(0\.9\.1|1\.0\.0)"$' "$TARGET/pyproject.toml"; then
  echo "Stage 10 expects the installed application to be version 0.9.1 or 1.0.0."
  echo "No change was made."
  exit 1
fi
...
ACTIVE_BEFORE=$(shasum -a 256 "$TARGET/data/app/releases/active.json" | awk '{print $1}')
STAMP=$(date +%Y%m%d-%H%M%S)
BACKUP_ROOT="$HOME/Desktop/Politica Backups"
BACKUP="$BACKUP_ROOT/Stage9.1-before-1.0.0-$STAMP"
mkdir -p "$BACKUP"
...
echo "$BACKUP"
echo
echo "Installing Stage 10 code without changing data/app, active releases, raw election data or .env..."
...
mkdir -p "$TARGET/dist"
for report in stage_10_build_manifest.json stage_10_test_report.json stage_10_integration_report.json; do
...
echo
echo "Running the complete Stage 4–10 regression suite..."
...
ACTIVE_AFTER=$(shasum -a 256 "$TARGET/data/app/releases/active.json" | awk '{print $1}')
if [ "$ACTIVE_AFTER" != "$ACTIVE_BEFORE" ]; then
  echo "The active release pointer changed unexpectedly; Stage 10 verification failed."
  exit 1
fi

echo
echo "Stage 10 v1.0.0 is installed and verified."
echo "Your active database, release history, raw sources, Google Sheets configuration and .env remain in place."
