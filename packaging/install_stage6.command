...
on_exit() {
  ...
  if [ "$status" -ne 0 ]; then
    echo
    echo "Stage 6 installation stopped before every verification passed."
...
if [ ! -d "$PAYLOAD/src/politica_erd" ] || [ ! -f "$PAYLOAD/pyproject.toml" ]; then
  echo "The Stage 6 payload is missing. Extract the complete ZIP before running this file."
  exit 1
fi
...
if ! grep -Eq '^version = "0\.(5|6)\.0"$' "$TARGET/pyproject.toml"; then
  echo "Stage 6 expects the installed application to be version 0.5.0 or 0.6.0."
  echo "No change was made."
  exit 1
fi
...
BACKUP="$BACKUP_ROOT/Stage5-before-0.6.0-$STAMP"
mkdir -p "$BACKUP"
...
echo "$BACKUP"
echo
echo "Installing Stage 6 code without changing data/app, active releases, raw election data or .env..."
...
mkdir -p "$TARGET/dist"
for report in stage_6_build_manifest.json stage_6_test_report.json stage_6_integration_report.json; do
...
echo
echo "Running Stage 4, Stage 5 and Stage 6 workflow tests..."
echo "A test line without 'ok' is still running; wait until Terminal prints the final OK."
uv run python -m unittest -v tests.test_stage4_workflow tests.test_stage5_workflow tests.test_stage6_workflow
...
echo
echo "Stage 6 v0.6.0 is installed and verified."
