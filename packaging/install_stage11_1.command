...
on_exit() {
  ...
  if [ "$status" -ne 0 ]; then
    echo
    echo "Stage 11.1 stopped before every verification passed."
...
for required in \
  ...
  if [ ! -e "$PAYLOAD/$required" ]; then
    echo "The Stage 11.1 payload is incomplete. Extract the complete ZIP before running this file."
...
if ! grep -Eq '^version = "1\.1\.(0|1)"$' "$TARGET/pyproject.toml"; then
  echo "Stage 11.1 expects the installed application to be version 1.1.0 or 1.1.1."
  echo "Install and verify Stage 11 first. No change was made."
  exit 1
fi
...
BACKUP="$BACKUP_ROOT/Stage11-before-1.1.1-$STAMP"
mkdir -p "$BACKUP"
...
echo "$BACKUP"
echo
echo "Installing Stage 11.1 code without changing data/app, active releases, raw election data or .env..."
...
mkdir -p "$TARGET/dist"
for report in stage_11_1_build_manifest.json stage_11_1_test_report.json stage_11_1_integration_report.json; do
...
echo
echo "Running the complete Stage 4–11.1 regression suite..."
...
if [ "$ACTIVE_BEFORE" != "$ACTIVE_AFTER" ]; then
  echo "The active release pointer changed unexpectedly; Stage 11.1 verification failed."
  exit 1
fi

echo
echo "Stage 11.1 v1.1.1 is installed and verified."
echo "All eight Senate states and territories now resolve across group, member and participation feeds."
