...
ADAPTER_ID = "adapter_aec_2019_v1"
...
HISTORICAL_2019_CONSTITUENCIES = (
    {
...
def ensure_2019_historical_constituencies(
    connection: duckdb.DuckDBPyConnection,
) -> int:
    """Add the 2019 division absent from the Stage 14.2 reference snapshot.

    North Sydney and Higgins are already governed by Stage 14.2. Stirling was
    abolished before 2022, while the current Corangamite reference reflects a
    later redistribution code. Their 2019 AEC codes therefore require exact
    historical crosswalk rows.
    """
    ...
    rows: list[tuple] = []
    for record in HISTORICAL_2019_CONSTITUENCIES:
        ...
        source_payload = {
            **record,
            ...
            "source": "HouseCandidatesDownload-24310.csv",
        }
        ...
        rows.append(
            (
                ...
                "Australia",
                "2019_federal_election",
                "aec_boundaries_applicable_to_2019_federal_election",
                None,
                None,
                None,
                f"Division of {record['constituency_name']}",
                None,
                "source_file_aec_2019_house_candidates",
                (
                    "HouseCandidatesDownload-24310.csv:DivisionID="
                ...
                "Codex",
                None,
                "Historical constituency governed by the final AEC 2019 candidate register.",
                record["official_constituency_code"],
                "published",
                source_row_hash,
                synced_at,
...
def _source_file_id(key: str) -> str:
    return f"source_file_aec_2019_{key}"
...
def _write_release_manifest(
    ...
    output = project_root / "dist" / "stage_14_3_2019_build_manifest.json"
...
def _validate_formal_checkpoint(project_root: Path, manifest: dict) -> None:
    expected = {
        "state_count": 8,
        "ballot_count": 14604925,
        "preference_count": 98547026,
        "file_count": 42,
    }
...
class ImportContext:
    ...
    formal_preference_chunk_sizes: dict[str, int] = field(
        default_factory=lambda: {"NSW": 250000}
    )
...
def register_sources(context: ImportContext) -> None:
    ...
    landing_rows = [
        (
            "source_landing_page_aec_2019_house",
            "authority_aec",
            ELECTION_ID,
            "2019 Federal Election House downloads",
        ...
        (
            "source_landing_page_aec_2019_senate",
            "authority_aec",
            ELECTION_ID,
            "2019 Federal Election Senate downloads",
    ...
    for source in manifest["sources"]:
        ...
        landing_id = (
            ...
            else "source_landing_page_aec_2019_senate"
        )
        file_rows.append(
            (
                file_id,
                ...
                "Australia",
                "current",
                "Registered by the governed AEC 2019 source catalogue.",
            )
        )
...
def seed_election(context: ImportContext) -> None:
    ...
    connection.execute(
        ...
        ('electoral_system_federal_house_irv_2019', 'irv', 'Instant-runoff voting', '2019 federal',
         'jurisdiction_aus_federal', DATE '2019-05-18', NULL, 1, 'full preferential', NULL,
         'sequential exclusion and transfer', 'AEC statutory rules', ?, 'House system for the 2019 federal election'),
        ('electoral_system_federal_senate_stv_2019', 'stv', 'Single transferable vote', '2019 federal',
         'jurisdiction_aus_federal', DATE '2019-05-18', NULL, NULL, 'optional preferential', 'Droop quota',
         'inclusive Gregory method', 'AEC statutory rules', ?, 'Senate system for the 2019 federal election')""",
        [context.revision_by_key["house_distribution"], context.revision_by_key["senate_distribution"]],
    )
    connection.execute(
        """INSERT INTO core.election VALUES
        (?, '24310', '2019 Australian federal election', DATE '2019-05-18', 2019,
    ...
    bulk_insert(
        ...
        [
            (HOUSE_CHAMBER_ID, ELECTION_ID, "chamber_house", "electoral_system_federal_house_irv_2019", 151, True, "final", "active"),
            (SENATE_CHAMBER_ID, ELECTION_ID, "chamber_senate", "electoral_system_federal_senate_stv_2019", 40, False, "final", "active"),
        ],
    )
    connection.execute(
        "INSERT INTO core.election_key_date VALUES (?, ?, 'polling_day', DATE '2019-05-18', 'official', ?)",
        [
            "election_key_date_fed_2019_polling_day",
            ELECTION_ID,
    ...
    for division_code, rows in sorted(house_by_division.items(), key=lambda item: int(item[0])):
        ...
        contest_rows.append(
            (contest, HOUSE_CHAMBER_ID, canonical_id, division_code, first.data["DivisionNm"], 1,
             "electoral_system_federal_house_irv_2019", "declared", False, None, "final", "active")
        )
    ...
    for state, rows in sorted(senate_by_state.items()):
        ...
        contest_rows.append(
            (contest, SENATE_CHAMBER_ID, None, state, STATE_NAMES[state], vacancies,
             "electoral_system_federal_senate_stv_2019", "declared", False, None, "final", "active")
        )
        ...
        if canonical_id not in seen_canonical:
            canonical_rows.append(
                (canonical_id, "authority_aec", f"polling_place:{official_code}", data["PollingPlaceNm"],
                 unit_type, datetime(2019, 5, 18).date(), None, "active")
            )
        ...
        if canonical_id not in seen_canonical:
            canonical_rows.append(
                (canonical_id, "authority_aec", f"division:{division}", data["DivisionNm"],
                 "district_total", datetime(2019, 5, 18).date(), None, "active")
            )
            seen_canonical.add(canonical_id)
        election_rows.append(
...
def import_external_identifiers(context: ImportContext) -> int:
    ...
    for source_row in context.rows("house_candidates"):
        ...
        if canonical_id:
            rows.append(
                (f"external_identifier_aec_2019_constituency_{data['DivisionID']}", "constituency", canonical_id,
                 "authority_aec", "aec_division_id", data["DivisionID"], datetime(2019, 5, 18).date(),
    ...
    for source_key in ("house_candidates", "senate_candidates"):
        for source_row in context.rows(source_key):
            ...
            if party_id:
                rows.append(
                    (f"external_identifier_aec_2019_party_{normalise_label(abbreviation).replace(' ', '_')}",
                     "party", party_id, "authority_aec", "aec_party_abbreviation", abbreviation,
                     datetime(2019, 5, 18).date(), None, context.revision_by_key[source_key],
    ...
    for source_key, chamber, key_field in (
        ...
        for source_row in context.rows(source_key):
            ...
            rows.append(
                (f"external_identifier_aec_2019_person_candidate_{data['CandidateID']}", "person", person_id,
                 "authority_aec", "aec_candidate_id", data["CandidateID"], datetime(2019, 5, 18).date(),
...
def reconcile(context: ImportContext) -> dict:
    ...
    add_check("house_formal_mismatches", house_formal_mismatches, 0, "rule_2019_house_formal_reconciliation")
    ...
    add_check("senate_formal_mismatches", senate_formal_mismatches, 0, "rule_2019_senate_formal_reconciliation")
    ...
    add_check("participation_mismatches", participation_mismatches, 0, "rule_2019_participation_reconciliation")
    ...
    add_check("missing_fact_lineage", lineage_missing, 0, "rule_2019_source_lineage")
    ...
    add_check("duplicate_vote_fact_grains", duplicate_facts, 0, "rule_2019_no_duplicate_facts")
    ...
    add_check("unmatched_house_constituencies", unmatched_house_constituencies, 0, "rule_2019_house_contests")
    ...
    add_check("unresolved_polling_result_units", null_polling_units, 0, "rule_2019_source_lineage")

    for source_key, expected in (
        ("house_tpp_state", 40),
        ("house_tpp_division", 755),
        ("house_tpp_division_vote_type", 3020),
        ("house_tpp_polling", 44375),
    ):
        add_check(
    ...
    add_check(
        ...
        0,
        "rule_2019_house_tpp_formal_reconciliation",
    )
    ...
    add_check(
        ...
        0,
        "rule_2019_house_tpp_vote_type_reconciliation",
    )
    ...
    add_check(
        ...
        0,
        "rule_2019_house_tpp_polling_reconciliation",
    )
    ...
    add_check(
        ...
        0,
        "rule_2019_house_tpp_state_reconciliation",
    )
    ...
    add_check(
        ...
        8,
        "rule_2019_formal_preference_sources",
    )
    add_check(
        "formal_senate_ballot_count",
        connection.execute("SELECT count(*) FROM ballot.ballot").fetchone()[0],
        14604925,
        "rule_2019_formal_ballot_count",
    )
    ...
    add_check(
        ...
        0,
        "rule_2019_formal_ballot_reconciliation",
    )
    add_check(
        "formal_ballot_invalid_preference_paths",
        connection.execute(
            "SELECT count(*) FROM ballot.ballot WHERE preference_count IS NULL OR preference_count < 1"
        ).fetchone()[0],
        0,
        "rule_2019_formal_preference_path",
    )
    ...
    connection.execute(
        "INSERT INTO audit.validation_run VALUES (?, ?, 'election', ?, '2019_federal_v1', ?, ?, ?, ?, ?, ?)",
...
def create_publication_and_derived(context: ImportContext, validation: dict) -> str | None:
    ...
    connection.execute(
        ...
        [snapshot_id, "2019 federal election final AEC release", now, now,
         "All 45 governed AEC final-result revisions retrieved in the Stage 2 manifest.", snapshot_hash,
...
def export_parquet(context: ImportContext) -> dict:
    ...
    root = context.project_root / "data" / "parquet" / "aec_2019" / "facts"
    if root.exists():
        shutil.rmtree(root)
    exports = []
    datasets = {
        ...
        for (revision,) in revisions:
            for chamber_code, chamber_id in (("house", HOUSE_CHAMBER_ID), ("senate", SENATE_CHAMBER_ID)):
                ...
                if not count:
                    continue
                directory = (
                    root / "jurisdiction_code=fed" / "election_year=2019" /
    ...
    output_manifest = context.project_root / "data" / "manifests" / "aec_2019_parquet.json"
...
def _reset_resume_downstream(context: ImportContext) -> None:
    ...
    connection.execute("DELETE FROM control.database_release WHERE release_id='release_0_2_0_aec_2019'")
    connection.execute("CHECKPOINT")


def resume_2019(database_path: Path, project_root: Path = PROJECT_ROOT) -> dict:
    manifest_path = project_root / "data" / "manifests" / "aec_2019_sources.json"
    ...
    try:
        ...
        if run is None:
            raise RuntimeError("No resumable 2019 import run matches the current source manifest.")
        ...
        formal_manifest = json.loads(
            (project_root / "data" / "manifests" / "aec_2019_formal_preferences.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        connection.execute(
            """INSERT INTO control.database_release VALUES
               ('release_0_2_0_aec_2019', '0.2.0', ?, ?, ?, 'Codex', ?)""",
            [
                "validated" if validation["status"] == "passed" else "blocked",
                started_at,
                completed_at if validation["status"] == "passed" else None,
                "2019 federal House, House TPP, Senate and formal-preference final-results import.",
            ],
        )
        table_counts = {
    ...
    report_path = project_root / "dist" / "stage_14_3_2019_import_report.json"
...
def import_2019(
    ...
    try:
        ...
        ensure_2019_historical_constituencies(connection)
        connection.execute(
...
def main() -> None:
    ...
    report = (
        resume_2019(args.database)
        if args.resume
        else import_2019(args.database, rebuild=not args.no_rebuild)
    )
