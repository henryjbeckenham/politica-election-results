...
HISTORICAL_2022_CONSTITUENCIES = (
    {
        "constituency_id": "constituency_federal_house_nsw_north_sydney_2022",
        "constituency_name": "North Sydney",
        "state_territory": "nsw",
        "official_constituency_code": "137",
    },
    {
        "constituency_id": "constituency_federal_house_vic_higgins_2022",
        "constituency_name": "Higgins",
        "state_territory": "vic",
        "official_constituency_code": "215",
    },
)


def ensure_2022_historical_constituencies(
    connection: duckdb.DuckDBPyConnection,
) -> int:
    """Add the two 2022 divisions absent from the current Grand snapshot.

    North Sydney and Higgins were valid AEC divisions at the 2022 election but
    were abolished before the current Grand Database constituency snapshot. The
    final AEC 2022 candidate register is the governing source for these
    ...
    rows: list[tuple] = []
    for record in HISTORICAL_2022_CONSTITUENCIES:
        ...
        source_payload = {
            **record,
            ...
            "source": "HouseCandidatesDownload-27966.csv",
        }
        ...
        rows.append(
            (
                ...
                "Australia",
                "2022_federal_election",
                "aec_boundaries_applicable_to_2022_federal_election",
                None,
                None,
                None,
                f"Division of {record['constituency_name']}",
                None,
                "source_file_aec_2022_house_candidates",
                (
                    "HouseCandidatesDownload-27966.csv:DivisionID="
                ...
                "Codex",
                None,
                "Historical constituency governed by the final AEC 2022 candidate register.",
                record["official_constituency_code"],
                "published",
                source_row_hash,
                synced_at,
...
def _source_file_id(key: str) -> str:
    return f"source_file_aec_2022_{key}"
...
def _write_release_manifest(
    ...
    output = project_root / "dist" / "stage_14_2_2022_build_manifest.json"
...
def _validate_formal_checkpoint(project_root: Path, manifest: dict) -> None:
    expected = {
        "state_count": 8,
        "ballot_count": 15040658,
        "preference_count": 101100266,
        "file_count": 35,
    }
...
def register_sources(context: ImportContext) -> None:
    ...
    landing_rows = [
        (
            "source_landing_page_aec_2022_house",
            "authority_aec",
            ELECTION_ID,
            "2022 Federal Election House downloads",
        ...
        (
            "source_landing_page_aec_2022_senate",
            "authority_aec",
            ELECTION_ID,
            "2022 Federal Election Senate downloads",
    ...
    for source in manifest["sources"]:
        ...
        landing_id = (
            ...
            else "source_landing_page_aec_2022_senate"
        )
        file_rows.append(
            (
                file_id,
                ...
                "Australia",
                "current",
                "Registered by the governed AEC 2022 source catalogue.",
            )
        )
...
def seed_election(context: ImportContext) -> None:
    ...
    connection.execute(
        ...
        ('electoral_system_federal_house_irv_2022', 'irv', 'Instant-runoff voting', '2022 federal',
         'jurisdiction_aus_federal', DATE '2022-05-21', NULL, 1, 'full preferential', NULL,
         'sequential exclusion and transfer', 'AEC statutory rules', ?, 'House system for the 2022 federal election'),
        ('electoral_system_federal_senate_stv_2022', 'stv', 'Single transferable vote', '2022 federal',
         'jurisdiction_aus_federal', DATE '2022-05-21', NULL, NULL, 'optional preferential', 'Droop quota',
         'inclusive Gregory method', 'AEC statutory rules', ?, 'Senate system for the 2022 federal election')""",
        [context.revision_by_key["house_distribution"], context.revision_by_key["senate_distribution"]],
    )
    connection.execute(
        """INSERT INTO core.election VALUES
        (?, '27966', '2022 Australian federal election', DATE '2022-05-21', 2022,
    ...
    bulk_insert(
        ...
        [
            (HOUSE_CHAMBER_ID, ELECTION_ID, "chamber_house", "electoral_system_federal_house_irv_2022", 151, True, "final", "active"),
            (SENATE_CHAMBER_ID, ELECTION_ID, "chamber_senate", "electoral_system_federal_senate_stv_2022", 40, False, "final", "active"),
        ],
    )
    connection.execute(
        "INSERT INTO core.election_key_date VALUES (?, ?, 'polling_day', DATE '2022-05-21', 'official', ?)",
        [
            "election_key_date_fed_2022_polling_day",
            ELECTION_ID,
    ...
    for division_code, rows in sorted(house_by_division.items(), key=lambda item: int(item[0])):
        ...
        contest_rows.append(
            (contest, HOUSE_CHAMBER_ID, canonical_id, division_code, first.data["DivisionNm"], 1,
             "electoral_system_federal_house_irv_2022", "declared", False, None, "final", "active")
        )
    ...
    for state, rows in sorted(senate_by_state.items()):
        ...
        contest_rows.append(
            (contest, SENATE_CHAMBER_ID, None, state, STATE_NAMES[state], vacancies,
             "electoral_system_federal_senate_stv_2022", "declared", False, None, "final", "active")
        )
...
def import_ballot_structure(context: ImportContext) -> None:
    ...
    for row in senate_rows:
        by_group[(row.data["StateAb"], row.data["Group"])].append(row)
    ...
    for (state, group_code), rows in sorted(by_group.items()):
        ...
        column_number = 21 if group_code == "UG" else ord(group_code) - ord("A") + 1
        if group_code != "UG":
        ...
        if canonical_id not in seen_canonical:
            canonical_rows.append(
                (canonical_id, "authority_aec", f"polling_place:{official_code}", data["PollingPlaceNm"],
                 unit_type, datetime(2022, 5, 21).date(), None, "active")
            )
        ...
        if canonical_id not in seen_canonical:
            canonical_rows.append(
                (canonical_id, "authority_aec", f"division:{division}", data["DivisionNm"],
                 "district_total", datetime(2022, 5, 21).date(), None, "active")
            )
            seen_canonical.add(canonical_id)
        election_rows.append(
    ...
    for source_row in source_rows_to_process:
        ...
        if chamber == "house":
            ...
            if ballot_position == 0:
                ...
                subject_id = context.ballot_groups.get((state, data["Group"]))
            else:
                subject_type = "candidacy"
...
def import_external_identifiers(context: ImportContext) -> int:
    ...
    for source_row in context.rows("house_candidates"):
        ...
        if canonical_id:
            rows.append(
                (f"external_identifier_aec_2022_constituency_{data['DivisionID']}", "constituency", canonical_id,
                 "authority_aec", "aec_division_id", data["DivisionID"], datetime(2022, 5, 21).date(),
    ...
    for source_key in ("house_candidates", "senate_candidates"):
        for source_row in context.rows(source_key):
            ...
            if party_id:
                rows.append(
                    (f"external_identifier_aec_2022_party_{normalise_label(abbreviation).replace(' ', '_')}",
                     "party", party_id, "authority_aec", "aec_party_abbreviation", abbreviation,
                     datetime(2022, 5, 21).date(), None, context.revision_by_key[source_key],
    ...
    for source_key, chamber, key_field in (
        ...
        for source_row in context.rows(source_key):
            ...
            rows.append(
                (f"external_identifier_aec_2022_person_candidate_{data['CandidateID']}", "person", person_id,
                 "authority_aec", "aec_candidate_id", data["CandidateID"], datetime(2022, 5, 21).date(),
...
def reconcile(context: ImportContext) -> dict:
    ...
    add_check("house_formal_mismatches", house_formal_mismatches, 0, "rule_2022_house_formal_reconciliation")
    ...
    add_check("senate_formal_mismatches", senate_formal_mismatches, 0, "rule_2022_senate_formal_reconciliation")
    ...
    add_check("participation_mismatches", participation_mismatches, 0, "rule_2022_participation_reconciliation")
    ...
    add_check("missing_fact_lineage", lineage_missing, 0, "rule_2022_source_lineage")
    ...
    add_check("duplicate_vote_fact_grains", duplicate_facts, 0, "rule_2022_no_duplicate_facts")
    ...
    add_check("unmatched_house_constituencies", unmatched_house_constituencies, 0, "rule_2022_house_contests")
    ...
    add_check("unresolved_polling_result_units", null_polling_units, 0, "rule_2022_source_lineage")

    for source_key, expected in (
        ("house_tpp_state", 40),
        ("house_tpp_division", 755),
        ("house_tpp_division_vote_type", 3020),
        ("house_tpp_polling", 42395),
    ):
        add_check(
    ...
    add_check(
        ...
        0,
        "rule_2022_house_tpp_formal_reconciliation",
    )
    ...
    add_check(
        ...
        0,
        "rule_2022_house_tpp_vote_type_reconciliation",
    )
    ...
    add_check(
        ...
        0,
        "rule_2022_house_tpp_polling_reconciliation",
    )
    ...
    add_check(
        ...
        0,
        "rule_2022_house_tpp_state_reconciliation",
    )
    ...
    add_check(
        ...
        0,
        "rule_2022_formal_ballot_reconciliation",
    )
    add_check(
        ...
        0,
        "rule_2022_formal_preference_path",
    )
    ...
    connection.execute(
        "INSERT INTO audit.validation_run VALUES (?, ?, 'election', ?, '2022_federal_v1', ?, ?, ?, ?, ?, ?)",
...
def create_publication_and_derived(context: ImportContext, validation: dict) -> str | None:
    ...
    connection.execute(
        ...
        [snapshot_id, "2022 federal election final AEC release", now, now,
...
def export_parquet(context: ImportContext) -> dict:
    ...
    root = context.project_root / "data" / "parquet" / "aec_2022" / "facts"
    if root.exists():
        shutil.rmtree(root)
    exports = []
    datasets = {
        ...
        for (revision,) in revisions:
            for chamber_code, chamber_id in (("house", HOUSE_CHAMBER_ID), ("senate", SENATE_CHAMBER_ID)):
                ...
                if not count:
                    continue
                directory = (
                    root / "jurisdiction_code=fed" / "election_year=2022" /
    ...
    output_manifest = context.project_root / "data" / "manifests" / "aec_2022_parquet.json"
...
def _reset_resume_downstream(context: ImportContext) -> None:
    ...
    connection.execute("DELETE FROM control.database_release WHERE release_id='release_0_2_0_aec_2022'")
    connection.execute("CHECKPOINT")


def resume_2022(database_path: Path, project_root: Path = PROJECT_ROOT) -> dict:
    manifest_path = project_root / "data" / "manifests" / "aec_2022_sources.json"
    ...
    try:
        ...
        if run is None:
            raise RuntimeError("No resumable 2022 import run matches the current source manifest.")
        ...
        formal_manifest = json.loads(
            (project_root / "data" / "manifests" / "aec_2022_formal_preferences.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        connection.execute(
            """INSERT INTO control.database_release VALUES
               ('release_0_2_0_aec_2022', '0.2.0', ?, ?, ?, 'Codex', ?)""",
            [
                "validated" if validation["status"] == "passed" else "blocked",
                started_at,
                completed_at if validation["status"] == "passed" else None,
                "2022 federal House, House TPP, Senate and formal-preference final-results import.",
            ],
        )
        table_counts = {
    ...
    report_path = project_root / "dist" / "stage_14_2_2022_import_report.json"
...
def import_2022(
    ...
    try:
        ...
        connection.execute("SET preserve_insertion_order=false")
        connection.execute("SET threads=4")
        ensure_2022_historical_constituencies(connection)
        connection.execute(
...
def main() -> None:
    ...
    report = (
        resume_2022(args.database)
        if args.resume
        else import_2022(args.database, rebuild=not args.no_rebuild)
    )
