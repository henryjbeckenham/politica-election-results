...
from .formal_preferences import import_formal_preferences
from .ids import (
    candidacy_id,
    contest_id,
    deterministic_uuid,
...
HISTORICAL_2016_CONSTITUENCIES = (
    {
        "constituency_id": "constituency_federal_house_sa_port_adelaide_2016",
        "constituency_name": "Port Adelaide",
        "state_territory": "sa",
        "official_constituency_code": "189",
    },
    {
        "constituency_id": "constituency_federal_house_sa_wakefield_2016",
        "constituency_name": "Wakefield",
        "state_territory": "sa",
        "official_constituency_code": "191",
    },
    {
        "constituency_id": "constituency_federal_house_tas_denison_2016",
        "constituency_name": "Denison",
        "state_territory": "tas",
        "official_constituency_code": "194",
    },
    {
        "constituency_id": "constituency_federal_house_vic_batman_2016",
        "constituency_name": "Batman",
        "state_territory": "vic",
        "official_constituency_code": "199",
    },
    {
        "constituency_id": "constituency_federal_house_vic_corangamite_2019",
        "constituency_name": "Corangamite",
        "state_territory": "vic",
        "official_constituency_code": "207",
    },
    {
        "constituency_id": "constituency_federal_house_vic_mcmillan_2016",
        "constituency_name": "McMillan",
        "state_territory": "vic",
        "official_constituency_code": "227",
    },
    {
        "constituency_id": "constituency_federal_house_vic_melbourne_ports_2016",
        "constituency_name": "Melbourne Ports",
        "state_territory": "vic",
        "official_constituency_code": "230",
    },
    {
        "constituency_id": "constituency_federal_house_vic_murray_2016",
        "constituency_name": "Murray",
        "state_territory": "vic",
        "official_constituency_code": "231",
    },
    {
        "constituency_id": "constituency_federal_house_wa_stirling_2019",
        "constituency_name": "Stirling",
        "state_territory": "wa",
        "official_constituency_code": "246",
    },
)
...
def seed_packaged_reference_snapshot(
    ...
    """Reuse the exact canonical reference snapshot carried by Stage 14.2."""

    counts: dict[str, int] = {}
    for table in ("person", "party", "constituency"):
        source = project_root / "data" / "stage14_2" / "tables" / "sync" / f"{table}.parquet"
        if not source.is_file():
            raise RuntimeError(
                f"The Stage 14.2 canonical reference shard is missing: {source}"
            )
...
def ensure_2016_historical_constituencies(
    connection: duckdb.DuckDBPyConnection,
) -> int:
    """Add divisions absent from the Stage 14.2 reference snapshot.

    North Sydney and Higgins are already governed by Stage 14.2. The nine
    records here either ceased to exist or changed official identity after the
    2016 election. Corangamite and Stirling deliberately reuse their governed
    Stage 14.3 identifiers so the combined release has one historical crosswalk
    for each official AEC division code.
    """
    ...
    rows: list[tuple] = []
    for record in HISTORICAL_2016_CONSTITUENCIES:
        if connection.execute(
            "SELECT 1 FROM sync.constituency WHERE official_constituency_code=?",
        ...
        source_payload = {
            **record,
            ...
            "source": "HouseCandidatesDownload-20499.csv",
        }
        ...
        rows.append(
            (
                ...
                "Australia",
                "2016_federal_election",
                "aec_boundaries_applicable_to_2016_federal_election",
                None,
                None,
                None,
                f"Division of {record['constituency_name']}",
                None,
                "source_file_aec_2016_house_candidates",
                (
                    "HouseCandidatesDownload-20499.csv:DivisionID="
                ...
                "Codex",
                None,
                "Historical constituency governed by the final AEC 2016 candidate register.",
                record["official_constituency_code"],
                "published",
                source_row_hash,
                synced_at,
...
def _source_file_id(key: str) -> str:
    return f"source_file_aec_2016_{key}"
...
def _write_release_manifest(
    ...
    manifest = {
        ...
        "database_path": str(database_path.resolve().relative_to(project_root.resolve())),
    ...
    output = project_root / "dist" / "stage_14_4_2016_build_manifest.json"
...
def _validate_formal_checkpoint(project_root: Path, manifest: dict) -> None:
    expected = {
        "state_count": 8,
        "ballot_count": 13838900,
        "preference_count": 93842251,
        "file_count": 53,
    }
...
def register_sources(context: ImportContext) -> None:
    ...
    landing_rows = [
        (
            "source_landing_page_aec_2016_house",
            "authority_aec",
            ELECTION_ID,
            "2016 Federal Election House downloads",
        ...
        (
            "source_landing_page_aec_2016_senate",
            "authority_aec",
            ELECTION_ID,
            "2016 Federal Election Senate downloads",
    ...
    for source in manifest["sources"]:
        ...
        landing_id = (
            ...
            else "source_landing_page_aec_2016_senate"
        )
        file_rows.append(
            (
                file_id,
                ...
                "Australia",
                "current",
                "Registered by the governed AEC 2016 source catalogue.",
            )
        )
...
def stage_sources(context: ImportContext) -> int:
    ...
    for key, source in context.source_by_key.items():
        if source.get("family") == "formal_preferences":
            # These archives contain 15.9 million wide ballot rows. The immutable
            # ZIPs are the raw layer; their governed normalisation is written
            # directly to partitioned Parquet rather than duplicated as JSON.
            continue
...
def seed_election(context: ImportContext) -> None:
    ...
    connection.execute(
        ...
        ('electoral_system_federal_house_irv_2016', 'irv', 'Instant-runoff voting', '2016 federal',
         'jurisdiction_aus_federal', DATE '2016-07-02', NULL, 1, 'full preferential', NULL,
         'sequential exclusion and transfer', 'AEC statutory rules', ?, 'House system for the 2016 federal election'),
        ('electoral_system_federal_senate_stv_2016', 'stv', 'Single transferable vote', '2016 federal',
         'jurisdiction_aus_federal', DATE '2016-07-02', NULL, NULL, 'optional preferential', 'Droop quota',
         'inclusive Gregory method', 'AEC statutory rules', ?, 'Senate system for the 2016 federal election')""",
        [context.revision_by_key["house_distribution"], context.revision_by_key["senate_distribution"]],
    )
    connection.execute(
        """INSERT INTO core.election VALUES
        (?, '20499', '2016 Australian federal election', DATE '2016-07-02', 2016,
    ...
    bulk_insert(
        ...
        [
            (HOUSE_CHAMBER_ID, ELECTION_ID, "chamber_house", "electoral_system_federal_house_irv_2016", 150, True, "final", "active"),
            (SENATE_CHAMBER_ID, ELECTION_ID, "chamber_senate", "electoral_system_federal_senate_stv_2016", 76, False, "final", "active"),
        ],
    )
    connection.execute(
        "INSERT INTO core.election_key_date VALUES (?, ?, 'polling_day', DATE '2016-07-02', 'official', ?)",
        [
            "election_key_date_fed_2016_polling_day",
            ELECTION_ID,
...
def import_contests_and_candidates(context: ImportContext) -> None:
    ...
    for division_code, rows in sorted(house_by_division.items(), key=lambda item: int(item[0])):
        ...
        contest_rows.append(
            (contest, HOUSE_CHAMBER_ID, canonical_id, division_code, first.data["DivisionNm"], 1,
             "electoral_system_federal_house_irv_2016", "declared", False, None, "final", "active")
        )
    ...
    for state, rows in sorted(senate_by_state.items()):
        ...
        vacancies = 2 if state in {"ACT", "NT"} else 12
        contest_rows.append(
            (contest, SENATE_CHAMBER_ID, None, state, STATE_NAMES[state], vacancies,
             "electoral_system_federal_senate_stv_2016", "declared", False, None, "final", "active")
        )
        ...
        if canonical_id not in seen_canonical:
            canonical_rows.append(
                (canonical_id, "authority_aec", f"polling_place:{official_code}", data["PollingPlaceNm"],
                 unit_type, datetime(2016, 5, 18).date(), None, "active")
            )
        ...
        if canonical_id not in seen_canonical:
            canonical_rows.append(
                (canonical_id, "authority_aec", f"division:{division}", data["DivisionNm"],
                 "district_total", datetime(2016, 5, 18).date(), None, "active")
            )
            seen_canonical.add(canonical_id)
        election_rows.append(
...
def import_vote_results(context: ImportContext) -> int:
    ...
    inserted += _checkpointed_vote_source(
        ...
        lambda: _insert_tpp_division_vote_type(context),
    )
    inserted += _checkpointed_vote_source(
        context,
        "house_tpp_polling",
...
def import_outcomes(context: ImportContext) -> int:
    ...
    for source_row in context.rows("senate_elected"):
        ...
        outcome_id = deterministic_uuid("contest_outcome", contest, candidate, "elected")
        outcome_rows.append(
            (outcome_id, contest, candidate, "elected", order, None, "final",
...
def import_external_identifiers(context: ImportContext) -> int:
    ...
    for source_row in context.rows("house_candidates"):
        ...
        if canonical_id:
            rows.append(
                (f"external_identifier_aec_2016_constituency_{data['DivisionID']}", "constituency", canonical_id,
                 "authority_aec", "aec_division_id", data["DivisionID"], datetime(2016, 5, 18).date(),
    ...
    for source_key in ("house_candidates", "senate_candidates"):
        for source_row in context.rows(source_key):
            ...
            if party_id:
                rows.append(
                    (f"external_identifier_aec_2016_party_{normalise_label(abbreviation).replace(' ', '_')}",
                     "party", party_id, "authority_aec", "aec_party_abbreviation", abbreviation,
                     datetime(2016, 5, 18).date(), None, context.revision_by_key[source_key],
    ...
    for source_key, chamber, key_field in (
        ...
        for source_row in context.rows(source_key):
            ...
            rows.append(
                (f"external_identifier_aec_2016_person_candidate_{data['CandidateID']}", "person", person_id,
                 "authority_aec", "aec_candidate_id", data["CandidateID"], datetime(2016, 5, 18).date(),
...
def reconcile(context: ImportContext) -> dict:
    ...
    add_check("house_formal_mismatches", house_formal_mismatches, 0, "rule_2016_house_formal_reconciliation")
    ...
    add_check("senate_formal_mismatches", senate_formal_mismatches, 0, "rule_2016_senate_formal_reconciliation")
    ...
    add_check("participation_mismatches", participation_mismatches, 0, "rule_2016_participation_reconciliation")
    ...
    add_check("missing_fact_lineage", lineage_missing, 0, "rule_2016_source_lineage")
    ...
    add_check("duplicate_vote_fact_grains", duplicate_facts, 0, "rule_2016_no_duplicate_facts")
    ...
    add_check("unmatched_house_constituencies", unmatched_house_constituencies, 0, "rule_2016_house_contests")
    ...
    add_check("unresolved_polling_result_units", null_polling_units, 0, "rule_2016_source_lineage")

    for source_key, expected in (
        ("house_tpp_state", 40),
        ("house_tpp_division", 750),
        ("house_tpp_division_vote_type", 3000),
        ("house_tpp_polling", 41640),
    ):
        add_check(
    ...
    add_check(
        ...
        0,
        "rule_2016_house_tpp_formal_reconciliation",
    )

    tpp_vote_type_mismatches = connection.execute(
        """
        WITH detail AS (
          SELECT contest_id, party_id, sum(integer_value) AS votes
          FROM results.vote_result
          WHERE source_revision_id=? AND result_type='tpp' AND measure_type='votes'
          GROUP BY contest_id, party_id
        ), totals AS (
          SELECT contest_id, party_id, integer_value AS votes
          FROM results.vote_result
          WHERE source_revision_id=? AND result_type='tpp'
            AND vote_type='total' AND measure_type='votes'
        )
        SELECT count(*) FROM detail FULL OUTER JOIN totals USING (contest_id, party_id)
        WHERE detail.votes IS DISTINCT FROM totals.votes
        """,
        [
            context.revision_by_key["house_tpp_division_vote_type"],
            context.revision_by_key["house_tpp_division"],
        ],
    ).fetchone()[0]
    add_check(
        "house_tpp_vote_type_mismatches",
        tpp_vote_type_mismatches,
        0,
        "rule_2016_house_tpp_vote_type_reconciliation",
    )
    ...
    add_check(
        ...
        0,
        "rule_2016_house_tpp_polling_reconciliation",
    )
    ...
    add_check(
        ...
        0,
        "rule_2016_house_tpp_state_reconciliation",
    )
    ...
    add_check(
        ...
        8,
        "rule_2016_formal_preference_sources",
    )
    add_check(
        ...
        13838900,
        "rule_2016_formal_ballot_count",
    )
    formal_ballot_mismatches = connection.execute(
        """
        WITH published AS (
        ...
        WHERE d.row_count <> p.formal_votes
        """
    ).fetchone()[0]
    add_check(
        "formal_ballot_state_mismatches",
        formal_ballot_mismatches,
        0,
        "rule_2016_formal_ballot_reconciliation",
    )
    add_check(
        ...
        0,
        "rule_2016_formal_preference_path",
    )
    ...
    connection.execute(
        "INSERT INTO audit.validation_run VALUES (?, ?, 'election', ?, '2016_federal_v1', ?, ?, ?, ?, ?, ?)",
...
def create_publication_and_derived(context: ImportContext, validation: dict) -> str | None:
    ...
    connection.execute(
        ...
        [snapshot_id, "2016 federal election final AEC release", now, now,
         "All 46 governed AEC final-result revisions retrieved in the Stage 14.4 manifest.", snapshot_hash,
...
def export_parquet(context: ImportContext) -> dict:
    ...
    root = context.project_root / "data" / "parquet" / "aec_2016" / "facts"
    if root.exists():
        shutil.rmtree(root)
    exports = []
    datasets = {
        ...
        for (revision,) in revisions:
            for chamber_code, chamber_id in (("house", HOUSE_CHAMBER_ID), ("senate", SENATE_CHAMBER_ID)):
                ...
                if not count:
                    continue
                directory = (
                    root / "jurisdiction_code=fed" / "election_year=2016" /
    ...
    output_manifest = context.project_root / "data" / "manifests" / "aec_2016_parquet.json"
...
def _reset_resume_downstream(context: ImportContext) -> None:
    ...
    connection.execute("DELETE FROM control.database_release WHERE release_id='release_0_2_0_aec_2016'")
    connection.execute("CHECKPOINT")
...
def _reset_validation_and_publication(context: ImportContext) -> None:
    ...
    connection.execute(
        ...
        "WHERE release_id='release_0_2_0_aec_2016'"
    )
    connection.execute("CHECKPOINT")


def resume_2016(database_path: Path, project_root: Path = PROJECT_ROOT) -> dict:
    manifest_path = project_root / "data" / "manifests" / "aec_2016_sources.json"
    ...
    try:
        ...
        if run is None:
            raise RuntimeError("No resumable 2016 import run matches the current source manifest.")
        ...
        formal_manifest_path = (
            project_root / "data" / "manifests" / "aec_2016_formal_preferences.json"
        )
        try:
            ...
            formal_manifest = import_formal_preferences(context)
        ...
        if completed_fact_counts == expected_completed_fact_counts:
            ...
            house_counts = (844, 6092, 3590)
            senate_counts = (4112, 452193, 27458)
            external_identifier_count = 240
        else:
        ...
        connection.execute(
            """INSERT INTO control.database_release VALUES
               ('release_0_2_0_aec_2016', '0.2.0', ?, ?, ?, 'Codex', ?)""",
            [
                "validated" if validation["status"] == "passed" else "blocked",
                started_at,
                completed_at if validation["status"] == "passed" else None,
                "2016 federal House, House TPP, Senate and formal-preference final-results import.",
            ],
        )
        table_counts = {
    ...
    report_path = project_root / "dist" / "stage_14_4_2016_import_report.json"
...
def import_2016(
    ...
    try:
        ...
        ensure_2016_historical_constituencies(connection)
        connection.execute(
        ...
        print("[1/12] Registering immutable source revisions", flush=True)
        register_sources(context)
        print("[2/12] Preserving source-native staging rows", flush=True)
        staged_count = stage_sources(context)
        print("[3/12] Creating election, contests and candidacies", flush=True)
        seed_election(context)
        import_contests_and_candidates(context)
        print("[4/12] Creating ballot structure and reporting units", flush=True)
        import_ballot_structure(context)
        import_reporting_units(context)
        print("[5/12] Transforming Senate formal-preference ballots", flush=True)
        formal_preference_manifest = import_formal_preferences(context)
        refresh_data_dictionary(connection, "0.2.0")
        export_catalogues(connection, project_root)
        print("[6/12] Importing vote facts", flush=True)
        vote_result_count = import_vote_results(context)
        print("[7/12] Importing participation and outcomes", flush=True)
        participation_count = import_participation(context)
        outcome_count = import_outcomes(context)
        print("[8/12] Importing House and Senate count rounds", flush=True)
        house_counts = import_house_counts(context)
        senate_counts = import_senate_counts(context)
        print("[9/12] Finalising external identifiers and lineage", flush=True)
        external_identifier_count = import_external_identifiers(context)
        _flush(connection, "provenance.row_lineage", context.lineage_rows)
        print("[10/12] Running blocking reconciliations", flush=True)
        validation = reconcile(context)
        print("[11/12] Creating publication and derived snapshots", flush=True)
        publication_snapshot_id = create_publication_and_derived(context, validation)
        connection.execute("CHECKPOINT")
        print("[12/12] Exporting partitioned Parquet facts", flush=True)
        ...
        connection.execute(
            ...
            [completed_at, "validated" if validation["status"] == "passed" else "failed_validation",
             ...
             f"Stage 2 import completed with {validation['warning_count']} unresolved canonical party-label warnings.",
             import_run_id],
        )
        connection.execute(
            """INSERT INTO control.database_release VALUES
...
def main() -> None:
    ...
    report = (
        resume_2016(args.database)
        if args.resume
        else import_2016(args.database, rebuild=not args.no_rebuild)
    )
