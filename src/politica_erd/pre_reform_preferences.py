...
if TYPE_CHECKING:
    from .import_2013 import ImportContext
...
def _archive_member(path: Path) -> str:
    with zipfile.ZipFile(path) as archive:
        ...
        if "Event:17496" not in metadata:
            raise ValueError(f"Unexpected 2013 BTL metadata in {path.name}.")
        if headers[:4] != ["CandidateId", "Preference", "Batch", "Paper"]:
            raise ValueError(f"Unexpected 2013 BTL columns in {path.name}: {headers}")
        return member
...
def _official_non_ticket_votes(context: "ImportContext") -> dict[str, int]:
    ...
    if set(values) != set(BTL_PREFERENCE_SOURCES):
        raise ValueError("The official 2013 non-ticket vote totals do not cover all states.")
    return values
...
def _write_state(
    ...
    if source_rows != expected_ballots * len(candidate_map):
        raise ValueError(
            f"The {state} BTL matrix has {source_rows:,} source rows; expected "
            f"{expected_ballots:,} ballots x {len(candidate_map)} candidates."
        )
    ...
    with tempfile.TemporaryDirectory(prefix=f"politica-2013-btl-{state.lower()}-") as temporary:
        ...
        if ballot_count != expected_ballots:
            raise ValueError(
                f"The {state} BTL transform produced {ballot_count:,} ballots; "
                f"expected {expected_ballots:,}."
            )
...
def import_pre_reform_preferences(context: "ImportContext") -> dict:
    """Import the directly marked 2013 BTL papers and activate shared views.
    ...
    connection = context.connection
    expected_by_state = _official_non_ticket_votes(context)
    parquet_root = context.project_root / "data" / "parquet" / "aec_2013" / "formal_preferences"
    ...
    for state, source_key in BTL_PREFERENCE_SOURCES.items():
        ...
        revision = context.revision_by_key[source_key]
        expected_ballots = expected_by_state[state]
        dataset_id = deterministic_uuid(
            "ballot_dataset", context.senate_contests[state], revision
        )
        dataset_rows.append(
            (
                ...
                revision,
                f"{state} 2013 non-ticket formal Senate ballot papers",
                "below_the_line",
                "AEC batch and paper numbers retained only as anonymous source coordinates",
                "No elector identity is present; group-ticket ballots exist only as official aggregates.",
                "aec_2013_btl_preferences_v1",
                expected_ballots,
                "active",
            )
        )
        ...
        if existing_files:
            ...
            if existing[0] == expected_ballots:
                print(f"      reusing validated {state} 2013 BTL preferences", flush=True)
                state_preference_counts[state] = int(existing[1] or 0)
                continue
        print(f"      transforming {state} 2013 BTL preferences", flush=True)
    ...
    ballot_count = sum(expected_by_state.values())
    ...
    manifest = {
        ...
        "formal_vote_count": ticket_votes + ballot_count,
        "file_count": len(files),
        "state_ballot_counts": expected_by_state,
        "state_preference_counts": state_preference_counts,
        "files": files,
    }
    manifest_path = context.project_root / "data" / "manifests" / "aec_2013_formal_preferences.json"
