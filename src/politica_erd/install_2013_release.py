"""Create and activate the immutable Stage 14.5 multi-election release."""
...
def _copy_2013_artifacts(project_root: Path, release_root: Path) -> None:
    source_parquet = project_root / "data" / "parquet" / "aec_2013"
    source_raw = project_root / "data" / "raw" / "aec" / "2013_federal"
    if not source_parquet.is_dir() or not any(source_parquet.rglob("*.parquet")):
        raise HistoricalReleaseError("The verified 2013 formal-ballot Parquet set is missing.")
    if not source_raw.is_dir() or not any(source_raw.rglob("*")):
        raise HistoricalReleaseError("The governed 2013 raw-source set is missing.")
    destination_parquet = release_root / "data" / "parquet" / "aec_2013"
    destination_raw = release_root / "data" / "raw" / "aec" / "2013_federal"
    shutil.copytree(
        source_parquet,
        destination_parquet,
    ...
    for source in sorted((project_root / "data" / "manifests").glob("aec_2013*")):
        if source.is_file():
...
def _verify_packaged_2013_artifacts(project_root: Path) -> None:
    manifest_root = project_root / "data" / "manifests"
    source_manifest = json.loads(
        (manifest_root / "aec_2013_sources.json").read_text(encoding="utf-8")
    )
    if (
        source_manifest.get("election_id") != ELECTION_2013
        or source_manifest.get("source_count") != 47
        or len(source_manifest.get("sources") or []) != 47
    ):
        raise HistoricalReleaseError("The governed 2013 source manifest is incomplete.")
    _verified_manifest_files(
        project_root,
        source_manifest,
        source_manifest["sources"],
        label="AEC 2013 source",
    )

    formal_manifest = json.loads(
        (manifest_root / "aec_2013_formal_preferences.json").read_text(
            encoding="utf-8"
        )
    )
    expected_formal = {
        "state_count": 8,
        "ballot_count": 471_030,
        "preference_count": 33_704_176,
        "above_the_line_ballot_count": 0,
        "below_the_line_ballot_count": 471_030,
        "group_ticket_vote_count": 12_941_989,
        "formal_vote_count": 13_413_019,
        "file_count": 95,
    }
    if {key: formal_manifest.get(key) for key in expected_formal} != expected_formal:
        raise HistoricalReleaseError(
            "The packaged 2013 formal-preference manifest does not reconcile."
        )
    _verified_manifest_files(
        project_root,
        formal_manifest,
        formal_manifest.get("files") or [],
        label="2013 formal-preference Parquet",
    )

    fact_manifest = json.loads(
        (manifest_root / "aec_2013_parquet.json").read_text(encoding="utf-8")
    )
    partitions = fact_manifest.get("partitions") or []
    if fact_manifest.get("partition_count") != len(partitions) or len(partitions) != 31:
        raise HistoricalReleaseError("The packaged 2013 fact-Parquet manifest is incomplete.")
    _verified_manifest_files(
        project_root,
        fact_manifest,
        partitions,
        label="2013 fact Parquet",
    )
...
def _merge_delta(
    ...
    migration = project_root / "schema" / "004_stage14_5_pre_reform_senate.sql"
    if not migration.is_file():
        raise HistoricalReleaseError("The Stage 14.5 additive schema migration is missing.")
    ...
    try:
        ...
        connection.execute(
            """INSERT OR IGNORE INTO control.database_release VALUES
               (?, ?, 'validated', ?, ?, 'Politica Stage 14.5 updater', ?)""",
            [
                release_id,
                schema_version,
                now,
                now,
                (
                    "Added the complete governed 2013 federal election alongside the "
                    "unchanged 2025, 2022, 2019 and 2016 elections, with election-specific "
                    "publication feeds."
                ),
            ],
        )
...
def _stage14_5_checks(
    ...
    expected_2013 = {
        ...
        "outcomes": 190,
        "count_rounds": 2525,
        "count_candidate_totals": 131071,
        "ballot_datasets": 8,
        "formal_ballots": 471030,
        "source_revisions": 47,
    }
    ...
    checks = {
        ...
        "2013_counts": (observed_2013, expected_2013),
    ...
    return {
        ...
        "stage": "stage_14_5_2013_multi_election_release",
        "checks": len(checks),
        "failures": failures,
...
def _existing_publication(
    ...
    publication = verify_historical_release_bundle(
        ...
        release_root,
        release_id,
        stage_manifest_name="stage_14_5_release.json",
        election_id=ELECTION_2013,
        election_validation_key="election_2013",
        expected_active_elections=5,
...
def install_2013_release(settings: AppSettings | None = None) -> dict:
    settings = settings or AppSettings.from_environment()
    service = JobService(settings)
    project_root = settings.project_root.resolve()
    delta_manifest_path = project_root / "data" / "manifests" / "aec_2013_delta_tables.json"
    import_report_path = project_root / "dist" / "stage_14_5_2013_import_report.json"
    if not delta_manifest_path.is_file() or not import_report_path.is_file():
        raise HistoricalReleaseError("The prevalidated Stage 14.5 import assets are incomplete.")
    ...
    if recorded_manifest_sha256 != observed_manifest_sha256:
        raise HistoricalReleaseError("The 2013 delta-table manifest failed checksum verification.")
    import_report = json.loads(import_report_path.read_text(encoding="utf-8"))
    if import_report.get("status") != "PASS":
        raise HistoricalReleaseError("The packaged 2013 import report is not passing.")
    _verify_packaged_2013_artifacts(project_root)

    with FileLock(str(settings.releases_root / ".stage14_5.lock")):
        ...
        try:
            ...
            if installed:
                return {
                    ...
                    "election_2013": _election_counts(base_connection, ELECTION_2013),
                }
            ...
            if not base_2016_counts["contests"] or not base_2016_sources:
                raise HistoricalReleaseError(
                    "Stage 14.5 requires the successfully installed Stage 14.4 2016 release."
                )
        finally:
        ...
        job_id = f"stage14_5_2013_ingestion_{token}"
        release_id = f"release_1_7_0_2013_{token}"
        release_root = settings.releases_root / f"politica-stage14-5-{token}"
        temporary = settings.releases_root / (
            f".politica-stage14-5-{token}.tmp-{uuid.uuid4().hex}"
        )
        ...
        try:
            ...
            _copy_2013_artifacts(project_root, temporary)
            ...
            try:
                ...
                stage_validation = _stage14_5_checks(
                    candidate,
                    base_2025_counts,
                    base_2025_sources,
                    base_2022_counts,
            ...
            verify_historical_candidate_database(
                ...
                temporary,
                election_id=ELECTION_2013,
                expected_counts=stage_validation["election_2013"],
                expected_active_elections=5,
                election_counts=_election_counts,
                error_type=HistoricalReleaseError,
            )
            if stage_validation["status"] != "PASS":
                raise HistoricalReleaseError(
                    "The merged multi-election release failed Stage 14.5 reconciliation: "
                    + json.dumps(stage_validation["failures"], ensure_ascii=False)
                )
            if _sha256_file(base_database) != base["database_sha256"]:
                raise HistoricalReleaseError(
                    "The immutable 2025, 2022, 2019 and 2016 base release changed during installation."
                )
            ...
            manifest_path = temporary / "data" / "manifests" / "stage_14_5_release.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Install the prevalidated Stage 14.5 2013 multi-election release."
    )
    parser.parse_args()
    print(json.dumps(install_2013_release(), indent=2))
