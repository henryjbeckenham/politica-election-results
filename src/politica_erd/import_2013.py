...
from .pre_reform_preferences import import_pre_reform_preferences
from .ids import (
    candidacy_id,
    contest_id,
    deterministic_uuid,
...
HISTORICAL_2013_CONSTITUENCIES = (
    {
...
def ensure_2013_historical_constituencies(
    connection: duckdb.DuckDBPyConnection,
) -> int:
    """Add the three 2013 division identities absent from Stage 14.4.
    ...
    election-specific canonical identities instead of being matched by code to
    ...
    rows: list[tuple] = []
    for record in HISTORICAL_2013_CONSTITUENCIES:
        ...
        source_payload = {
            **record,
            ...
            "source": "HouseCandidatesDownload-17496.csv",
        }
        ...
        rows.append(
            (
                ...
                "Australia",
                "2013_federal_election",
                "aec_boundaries_applicable_to_2013_federal_election",
                None,
                None,
                None,
                f"Division of {record['constituency_name']}",
                None,
                "source_file_aec_2013_house_candidates",
                (
                    "HouseCandidatesDownload-17496.csv:DivisionID="
                ...
                "Codex",
                None,
                "Historical constituency governed by the final AEC 2013 candidate register.",
...
def _source_file_id(key: str) -> str:
    return f"source_file_aec_2013_{key}"
...
def _write_release_manifest(
    ...
    output = project_root / "dist" / "stage_14_5_2013_build_manifest.json"
...
def _validate_formal_checkpoint(project_root: Path, manifest: dict) -> None:
    expected = {
        ...
        "below_the_line_ballot_count": 471030,
        "group_ticket_vote_count": 12941989,
        "formal_vote_count": 13413019,
    }
...
def register_sources(context: ImportContext) -> None:
    ...
    landing_rows = [
        (
            "source_landing_page_aec_2013_house",
            "authority_aec",
            ELECTION_ID,
            "2013 Federal Election House downloads",
        ...
        (
            "source_landing_page_aec_2013_senate",
            "authority_aec",
            ELECTION_ID,
            "2013 Federal Election Senate downloads",
    ...
    for source in manifest["sources"]:
        ...
        landing_id = (
            ...
            else "source_landing_page_aec_2013_senate"
        )
        file_rows.append(
            (
                file_id,
                ...
                "Australia",
                "current",
                "Registered by the governed AEC 2013 source catalogue.",
            )
        )
...
def seed_election(context: ImportContext) -> None:
    ...
    connection.execute(
        ...
        ('electoral_system_federal_house_irv_2013', 'irv', 'Instant-runoff voting', '2013 federal',
         'jurisdiction_aus_federal', DATE '2013-09-07', NULL, 1, 'full preferential', NULL,
         'sequential exclusion and transfer', 'AEC statutory rules', ?, 'House system for the 2013 federal election'),
        ('electoral_system_federal_senate_stv_2013', 'stv', 'Single transferable vote', '2013 federal',
         'jurisdiction_aus_federal', DATE '2013-09-07', NULL, NULL, 'full preferential below the line; group voting ticket above the line', 'Droop quota',
         'inclusive Gregory method with registered group voting tickets', 'AEC statutory rules', ?, 'Pre-reform Senate system for the 2013 federal election')""",
        [context.revision_by_key["house_distribution"], context.revision_by_key["senate_distribution"]],
    )
    connection.execute(
        """INSERT INTO core.election VALUES
        (?, '17496', '2013 Australian federal election', DATE '2013-09-07', 2013,
    ...
    bulk_insert(
        ...
        [
            (HOUSE_CHAMBER_ID, ELECTION_ID, "chamber_house", "electoral_system_federal_house_irv_2013", 150, True, "final", "active"),
            (SENATE_CHAMBER_ID, ELECTION_ID, "chamber_senate", "electoral_system_federal_senate_stv_2013", 40, False, "final", "active"),
        ],
    )
    connection.execute(
        "INSERT INTO core.election_key_date VALUES (?, ?, 'polling_day', DATE '2013-09-07', 'official', ?)",
        [
            "election_key_date_fed_2013_polling_day",
            ELECTION_ID,
    ...
    for division_code, rows in sorted(house_by_division.items(), key=lambda item: int(item[0])):
        ...
        contest_rows.append(
            (contest, HOUSE_CHAMBER_ID, canonical_id, division_code, first.data["DivisionNm"], 1,
             "electoral_system_federal_house_irv_2013", "declared", False, None, "final", "active")
        )
    ...
    for state, rows in sorted(senate_by_state.items()):
        ...
        contest_rows.append(
            (contest, SENATE_CHAMBER_ID, None, state, STATE_NAMES[state], vacancies,
             "electoral_system_federal_senate_stv_2013",
             "void" if state == "WA" else "declared", False, None, "final", "active")
        )
...
def import_group_voting_tickets(context: ImportContext) -> tuple[int, int]:
    """Normalise every registered 2013 Senate group voting ticket.
        ...
        if canonical_id not in seen_canonical:
            canonical_rows.append(
                (canonical_id, "authority_aec", f"polling_place:{official_code}", data["PollingPlaceNm"],
                 unit_type, datetime(2013, 9, 7).date(), None, "active")
            )
        ...
        if canonical_id not in seen_canonical:
            canonical_rows.append(
                (canonical_id, "authority_aec", f"division:{division}", data["DivisionNm"],
                 "district_total", datetime(2013, 9, 7).date(), None, "active")
            )
            seen_canonical.add(canonical_id)
        election_rows.append(
    ...
    for source_row in context.rows("senate_elected"):
        ...
        outcome_rows.append(
            (outcome_id, contest, candidate, "elected", order, None,
             "superseded" if data["StateAb"] == "WA" else "final",
...
def import_external_identifiers(context: ImportContext) -> int:
    ...
    for source_row in context.rows("house_candidates"):
        ...
        if canonical_id:
            rows.append(
                (f"external_identifier_aec_2013_constituency_{data['DivisionID']}", "constituency", canonical_id,
                 "authority_aec", "aec_division_id", data["DivisionID"], datetime(2013, 9, 7).date(),
    ...
    for source_key in ("house_candidates", "senate_candidates"):
        for source_row in context.rows(source_key):
            ...
            if party_id:
                rows.append(
                    (f"external_identifier_aec_2013_party_{normalise_label(abbreviation).replace(' ', '_')}",
                     "party", party_id, "authority_aec", "aec_party_abbreviation", abbreviation,
                     datetime(2013, 9, 7).date(), None, context.revision_by_key[source_key],
    ...
    for source_key, chamber, key_field in (
        ...
        for source_row in context.rows(source_key):
            ...
            rows.append(
                (f"external_identifier_aec_2013_person_candidate_{data['CandidateID']}", "person", person_id,
                 "authority_aec", "aec_candidate_id", data["CandidateID"], datetime(2013, 9, 7).date(),
...
def reconcile(context: ImportContext) -> dict:
    ...
    add_check("house_formal_mismatches", house_formal_mismatches, 0, "rule_2013_house_formal_reconciliation")
    ...
    add_check("senate_formal_mismatches", senate_formal_mismatches, 0, "rule_2013_senate_formal_reconciliation")
    ...
    add_check("participation_mismatches", participation_mismatches, 0, "rule_2013_participation_reconciliation")
    ...
    add_check("missing_fact_lineage", lineage_missing, 0, "rule_2013_source_lineage")
    ...
    add_check("duplicate_vote_fact_grains", duplicate_facts, 0, "rule_2013_no_duplicate_facts")
    ...
    add_check("unmatched_house_constituencies", unmatched_house_constituencies, 0, "rule_2013_house_contests")
    ...
    add_check("unresolved_polling_result_units", null_polling_units, 0, "rule_2013_source_lineage")

    for source_key, expected in (
        ("house_tpp_state", 40),
        ("house_tpp_division", 750),
        ("house_tpp_polling", 45730),
    ):
        add_check(
    ...
    add_check(
        ...
        0,
        "rule_2013_house_tpp_formal_reconciliation",
    )
    ...
    add_check(
        ...
        0,
        "rule_2013_house_tpp_polling_reconciliation",
    )
    ...
    add_check(
        ...
        0,
        "rule_2013_house_tpp_state_reconciliation",
    )
    ...
    add_check(
        ...
        8,
        "rule_2013_formal_preference_sources",
    )
    add_check(
    ...
    formal_ballot_mismatches = connection.execute(
        ...
        WHERE d.row_count <> p.non_ticket_votes
        """
    ).fetchone()[0]
    add_check(
        ...
        0,
        "rule_2013_formal_ballot_reconciliation",
    )
    add_check(
    ...
    add_check(
        ...
        0,
        "rule_2013_group_voting_ticket_reconciliation",
    )
    ...
    connection.execute(
        "INSERT INTO audit.validation_run VALUES (?, ?, 'election', ?, '2013_federal_v1', ?, ?, ?, ?, ?, ?)",
...
def create_publication_and_derived(context: ImportContext, validation: dict) -> str | None:
    ...
    connection.execute(
        ...
        [snapshot_id, "2013 federal election final AEC release", now, now,
         "All 47 governed AEC final-result revisions retrieved in the Stage 14.5 manifest.", snapshot_hash,
...
def export_parquet(context: ImportContext) -> dict:
    ...
    root = context.project_root / "data" / "parquet" / "aec_2013" / "facts"
    if root.exists():
        shutil.rmtree(root)
    exports = []
    datasets = {
        ...
        for (revision,) in revisions:
            for chamber_code, chamber_id in (("house", HOUSE_CHAMBER_ID), ("senate", SENATE_CHAMBER_ID)):
                ...
                if not count:
                    continue
                directory = (
                    root / "jurisdiction_code=fed" / "election_year=2013" /
    ...
    output_manifest = context.project_root / "data" / "manifests" / "aec_2013_parquet.json"
...
def _reset_resume_downstream(context: ImportContext) -> None:
    ...
    connection.execute("DELETE FROM control.database_release WHERE release_id='release_0_2_0_aec_2013'")
    connection.execute("CHECKPOINT")
...
def _reset_validation_and_publication(context: ImportContext) -> None:
    ...
    connection.execute(
        ...
        "WHERE release_id='release_0_2_0_aec_2013'"
    )
    connection.execute("CHECKPOINT")


def resume_2013(database_path: Path, project_root: Path = PROJECT_ROOT) -> dict:
    manifest_path = project_root / "data" / "manifests" / "aec_2013_sources.json"
    ...
    try:
        ...
        if run is None:
            raise RuntimeError("No resumable 2013 import run matches the current source manifest.")
        ...
        formal_manifest_path = (
            project_root / "data" / "manifests" / "aec_2013_formal_preferences.json"
        )
        try:
        ...
        if completed_fact_counts == expected_completed_fact_counts:
            ...
            house_counts = (1038, 8766, 5120)
            senate_counts = (1487, 122305, 9513)
        ...
        connection.execute(
            """INSERT INTO control.database_release VALUES
               ('release_0_2_0_aec_2013', '0.2.0', ?, ?, ?, 'Codex', ?)""",
            [
                "validated" if validation["status"] == "passed" else "blocked",
                started_at,
                completed_at if validation["status"] == "passed" else None,
                "2013 federal House, House TPP, Senate and formal-preference final-results import.",
            ],
        )
        table_counts = {
    ...
    report_path = project_root / "dist" / "stage_14_5_2013_import_report.json"
...
def import_2013(
    ...
    try:
        ...
        ensure_2013_historical_constituencies(connection)
        connection.execute(
        ...
        connection.execute(
            ...
            f"Stage 14.5 import completed with {validation['warning_count']} unresolved canonical party-label warnings.",
             import_run_id],
        )
        connection.execute(
            """INSERT INTO control.database_release VALUES
...
def main() -> None:
    ...
    report = (
        resume_2013(args.database)
        if args.resume
        else import_2013(args.database, rebuild=not args.no_rebuild)
    )
