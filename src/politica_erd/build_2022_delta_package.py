"""Build and export the prevalidated 2022 delta as portable table shards."""
...
from .import_2022 import import_2022
...
def export_delta_tables(
    ...
    output_root = project_root / "data" / "stage14_2" / "tables"
...
def build_delta_package(project_root: Path = PROJECT_ROOT) -> dict:
    database = project_root / "data" / "stage14_2" / "politica_2022_delta.duckdb"
    exported: dict = {}
    ...
    if import_report.get("status") != "PASS":
        raise RuntimeError("The 2022 import did not pass; no delta package was exported.")
    if not exported:
        raise RuntimeError("The 2022 delta tables were not exported before database close.")
