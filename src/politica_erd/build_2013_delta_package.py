"""Export the prevalidated 2013 delta as portable table shards."""
...
def export_delta_tables(
    ...
    output_root = project_root / "data" / "stage14_5" / "tables"
...
def export_existing_delta(
    ...
    database = database or (
        project_root / "data" / "stage14_5" / "politica_2013_delta.duckdb"
    )
    report_path = project_root / "dist" / "stage_14_5_2013_import_report.json"
    if not report_path.is_file():
        raise RuntimeError("The reconciled 2013 import report is missing.")
    report = json.loads(report_path.read_text(encoding="utf-8"))
    if report.get("status") != "PASS" or report.get("election_id") != ELECTION_ID:
        raise RuntimeError("The 2013 import has not passed; no delta will be exported.")
    blockers = [
        check
    ...
    if blockers:
        raise RuntimeError("The 2013 import report contains failed blockers.")
    if not database.is_file():
        raise RuntimeError(f"The reconciled 2013 database is missing: {database}")
    ...
    try:
        ...
        if election_count != 1:
            raise RuntimeError("The delta database does not contain the 2013 election.")
