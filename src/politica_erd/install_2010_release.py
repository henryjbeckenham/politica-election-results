"""Create and activate the immutable Stage 14.6 multi-election release."""
...
ELECTION_2010 = "election_fed_2010_08_21_general"
ELECTION_2013 = "election_fed_2013_09_07_general"
ELECTION_2016 = "election_fed_2016_07_02_general"
ELECTION_2019 = "election_fed_2019_05_18_general"
...
def _election_counts(
    ...
    return {
        ...
        "preference_transfers": connection.execute(
            """SELECT count(*) FROM "count".preference_transfer t
               JOIN "count".count_round r USING (count_round_id)
               JOIN core.contest c USING (contest_id)
               JOIN core.election_chamber ec USING (election_chamber_id)
               WHERE ec.election_id=?""",
        ...
        "group_voting_tickets": connection.execute(
            """SELECT count(*) FROM ballot.group_voting_ticket t
               JOIN core.contest c USING (contest_id)
               JOIN core.election_chamber ec USING (election_chamber_id)
               WHERE ec.election_id=?""",
            [election_id],
        ...
        "group_voting_ticket_preferences": connection.execute(
            """SELECT count(*) FROM ballot.group_voting_ticket_preference p
               JOIN ballot.group_voting_ticket t USING (group_voting_ticket_id)
               JOIN core.contest c USING (contest_id)
               JOIN core.election_chamber ec USING (election_chamber_id)
               WHERE ec.election_id=?""",
            [election_id],
...
def _copy_2010_artifacts(project_root: Path, release_root: Path) -> None:
    source_parquet = project_root / "data" / "parquet" / "aec_2010"
    source_raw = project_root / "data" / "raw" / "aec" / "2010_federal"
    if not source_parquet.is_dir() or not any(source_parquet.rglob("*.parquet")):
        raise HistoricalReleaseError("The verified 2010 formal-ballot Parquet set is missing.")
    if not source_raw.is_dir() or not any(source_raw.rglob("*")):
        raise HistoricalReleaseError("The governed 2010 raw-source set is missing.")
    destination_parquet = release_root / "data" / "parquet" / "aec_2010"
    destination_raw = release_root / "data" / "raw" / "aec" / "2010_federal"
    shutil.copytree(
        source_parquet,
        destination_parquet,
    ...
    for source in sorted((project_root / "data" / "manifests").glob("aec_2010*")):
        if source.is_file():
...
def _verify_packaged_2010_artifacts(project_root: Path) -> None:
    manifest_root = project_root / "data" / "manifests"
    source_manifest = json.loads(
        (manifest_root / "aec_2010_sources.json").read_text(encoding="utf-8")
    )
    if (
        source_manifest.get("election_id") != ELECTION_2010
        or source_manifest.get("source_count") != 47
        or len(source_manifest.get("sources") or []) != 47
    ):
        raise HistoricalReleaseError("The governed 2010 source manifest is incomplete.")
    _verified_manifest_files(
        project_root,
        source_manifest,
        source_manifest["sources"],
        label="AEC 2010 source",
    )

    formal_manifest = json.loads(
        (manifest_root / "aec_2010_formal_preferences.json").read_text(
            encoding="utf-8"
        )
    )
    expected_formal = {
        "state_count": 8,
        "ballot_count": 493_129,
        "preference_count": 23_659_799,
        "above_the_line_ballot_count": 0,
        "below_the_line_ballot_count": 493_129,
        "official_non_ticket_vote_count": 493_142,
        "unavailable_ballot_count": 13,
        "group_ticket_vote_count": 12_229_091,
        "formal_vote_count": 12_722_233,
        "represented_formal_vote_count": 12_722_220,
        "file_count": 95,
    }
    if {key: formal_manifest.get(key) for key in expected_formal} != expected_formal:
        raise HistoricalReleaseError(
            "The packaged 2010 formal-preference manifest does not reconcile."
        )
    _verified_manifest_files(
        project_root,
        formal_manifest,
        formal_manifest.get("files") or [],
        label="2010 formal-preference Parquet",
    )

    fact_manifest = json.loads(
        (manifest_root / "aec_2010_parquet.json").read_text(encoding="utf-8")
    )
    partitions = fact_manifest.get("partitions") or []
    if fact_manifest.get("partition_count") != len(partitions) or len(partitions) != 31:
        raise HistoricalReleaseError("The packaged 2010 fact-Parquet manifest is incomplete.")
    _verified_manifest_files(
        project_root,
        fact_manifest,
        partitions,
        label="2010 fact Parquet",
    )
...
def _merge_delta(
    ...
    migration = project_root / "schema" / "005_stage14_6_2010.sql"
    if not migration.is_file():
        raise HistoricalReleaseError("The Stage 14.6 additive schema migration is missing.")
    ...
    try:
        ...
        connection.execute(
            """INSERT OR IGNORE INTO control.database_release VALUES
               (?, ?, 'validated', ?, ?, 'Politica Stage 14.6 updater', ?)""",
            [
                release_id,
                schema_version,
                now,
                now,
                (
                    "Added the complete governed 2010 federal election alongside the "
                    "unchanged 2025, 2022, 2019, 2016 and 2013 elections, with election-specific "
                    "publication feeds."
                ),
            ],
        )
...
def _stage14_6_checks(
    connection: duckdb.DuckDBPyConnection,
    base_2025_counts: dict[str, int],
    base_2025_sources: str,
    base_2022_counts: dict[str, int],
    base_2022_sources: str,
    base_2019_counts: dict[str, int],
    base_2019_sources: str,
    base_2016_counts: dict[str, int],
    base_2016_sources: str,
    base_2013_counts: dict[str, int],
    base_2013_sources: str,
) -> dict:
    expected_2010 = {
        "contests": 158,
        "candidacies": 1198,
        "vote_results": 205788,
        "participation_results": 1896,
        "outcomes": 190,
        "count_rounds": 1875,
        "count_candidate_totals": 74400,
        "preference_transfers": 9239,
        "ballot_datasets": 8,
        "formal_ballots": 493129,
        "group_voting_tickets": 156,
        "group_voting_ticket_preferences": 9048,
        "source_revisions": 47,
    }
    observed_2010 = _election_counts(connection, ELECTION_2010)
    observed_2013 = _election_counts(connection, ELECTION_2013)
    observed_2016 = _election_counts(connection, ELECTION_2016)
    observed_2019 = _election_counts(connection, ELECTION_2019)
    ...
    checks = {
        ...
        "2010_counts": (observed_2010, expected_2010),
        "2013_counts_preserved": (observed_2013, base_2013_counts),
        "2013_source_identity_preserved": (
            _source_identity(connection, ELECTION_2013),
            base_2013_sources,
        ),
        "2016_counts_preserved": (observed_2016, base_2016_counts),
        "2016_source_identity_preserved": (
            _source_identity(connection, ELECTION_2016),
            base_2016_sources,
        ),
        "2019_counts_preserved": (observed_2019, base_2019_counts),
        "2019_source_identity_preserved": (
            _source_identity(connection, ELECTION_2019),
            base_2019_sources,
        ),
        "2022_counts_preserved": (observed_2022, base_2022_counts),
        "2022_source_identity_preserved": (
            _source_identity(connection, ELECTION_2022),
            base_2022_sources,
        ),
    ...
    return {
        ...
        "stage": "stage_14_6_2010_multi_election_release",
        "checks": len(checks),
        "failures": failures,
        "election_2010": observed_2010,
        "election_2013": observed_2013,
        "election_2016": observed_2016,
        "election_2019": observed_2019,
...
def _existing_publication(
    ...
    publication = verify_historical_release_bundle(
        ...
        release_root,
        release_id,
        stage_manifest_name="stage_14_6_release.json",
        election_id=ELECTION_2010,
        election_validation_key="election_2010",
        expected_active_elections=6,
...
def install_2010_release(settings: AppSettings | None = None) -> dict:
    settings = settings or AppSettings.from_environment()
    service = JobService(settings)
    project_root = settings.project_root.resolve()
    delta_manifest_path = project_root / "data" / "manifests" / "aec_2010_delta_tables.json"
    import_report_path = project_root / "dist" / "stage_14_6_2010_import_report.json"
    if not delta_manifest_path.is_file() or not import_report_path.is_file():
        raise HistoricalReleaseError("The prevalidated Stage 14.6 import assets are incomplete.")
    ...
    if recorded_manifest_sha256 != observed_manifest_sha256:
        raise HistoricalReleaseError("The 2010 delta-table manifest failed checksum verification.")
    import_report = json.loads(import_report_path.read_text(encoding="utf-8"))
    if import_report.get("status") != "PASS":
        raise HistoricalReleaseError("The packaged 2010 import report is not passing.")
    _verify_packaged_2010_artifacts(project_root)

    with FileLock(str(settings.releases_root / ".stage14_6.lock")):
        ...
        try:
            ...
            if installed:
                return {
                    ...
                    "election_2010": _election_counts(base_connection, ELECTION_2010),
                }
            ...
            base_2022_counts = _election_counts(base_connection, ELECTION_2022)
            base_2022_sources = _source_identity(base_connection, ELECTION_2022)
            base_2019_counts = _election_counts(base_connection, ELECTION_2019)
            base_2019_sources = _source_identity(base_connection, ELECTION_2019)
            base_2016_counts = _election_counts(base_connection, ELECTION_2016)
            base_2016_sources = _source_identity(base_connection, ELECTION_2016)
            base_2013_counts = _election_counts(base_connection, ELECTION_2013)
            base_2013_sources = _source_identity(base_connection, ELECTION_2013)
            if not base_2013_counts["contests"] or not base_2013_sources:
                raise HistoricalReleaseError(
                    "Stage 14.6 requires the successfully installed Stage 14.5 2013 release."
                )
        finally:
        ...
        job_id = f"stage14_6_2010_ingestion_{token}"
        release_id = f"release_1_8_0_2010_{token}"
        release_root = settings.releases_root / f"politica-stage14-6-{token}"
        temporary = settings.releases_root / (
            f".politica-stage14-6-{token}.tmp-{uuid.uuid4().hex}"
        )
        ...
        try:
            ...
            _copy_2010_artifacts(project_root, temporary)
            ...
            try:
                ...
                stage_validation = _stage14_6_checks(
                    candidate,
                    base_2025_counts,
                    base_2025_sources,
                    base_2022_counts,
            ...
            verify_historical_candidate_database(
                ...
                temporary,
                election_id=ELECTION_2010,
                expected_counts=stage_validation["election_2010"],
                expected_active_elections=6,
                election_counts=_election_counts,
                error_type=HistoricalReleaseError,
            )
            if stage_validation["status"] != "PASS":
                raise HistoricalReleaseError(
                    "The merged multi-election release failed Stage 14.6 reconciliation: "
                    + json.dumps(stage_validation["failures"], ensure_ascii=False)
                )
            if _sha256_file(base_database) != base["database_sha256"]:
                raise HistoricalReleaseError(
                    "The immutable 2025, 2022, 2019, 2016 and 2013 base release changed during installation."
                )
            ...
            manifest_path = temporary / "data" / "manifests" / "stage_14_6_release.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Install the prevalidated Stage 14.6 2010 multi-election release."
    )
    parser.parse_args()
    print(json.dumps(install_2010_release(), indent=2))
