"""Short post-installation smoke verification for Stage 14.4."""
...
from .install_2016_release import (
    ELECTION_2016,
    ELECTION_2019,
    ELECTION_2022,
    ELECTION_2025,
...
def verify() -> dict:
    ...
    checks = {
        "election_catalogue": (
            election_ids,
            [ELECTION_2025, ELECTION_2022, ELECTION_2019, ELECTION_2016],
        ),
        "2025_house_seats": (
            feeds.build(
        ...
        "2016_house_seats": (
            feeds.build(
                "house_seat_results", PublicationFilters(election_id=ELECTION_2016)
            ).row_count,
            150,
        ),
        "2019_house_seats": (
        ...
        "2016_senate_view": (
            feeds.build(
                "senate_composition", PublicationFilters(election_id=ELECTION_2016)
            ).row_count,
            76,
        ),
        "2019_senate_view": (
        ...
        "2016_boundaries": (
            visualisations.catalogue(ELECTION_2016)["boundary_geometry"]["feature_count"],
            150,
        ),
        "2019_boundaries": (
