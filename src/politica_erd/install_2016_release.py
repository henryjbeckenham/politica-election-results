"""Create and activate the immutable Stage 14.4 multi-election release."""
...
def _copy_2016_artifacts(project_root: Path, release_root: Path) -> None:
    source_parquet = project_root / "data" / "parquet" / "aec_2016"
    source_raw = project_root / "data" / "raw" / "aec" / "2016_federal"
    if not source_parquet.is_dir() or not any(source_parquet.rglob("*.parquet")):
        raise HistoricalReleaseError("The verified 2016 formal-ballot Parquet set is missing.")
    if not source_raw.is_dir() or not any(source_raw.rglob("*")):
        raise HistoricalReleaseError("The governed 2016 raw-source set is missing.")
    destination_parquet = release_root / "data" / "parquet" / "aec_2016"
    destination_raw = release_root / "data" / "raw" / "aec" / "2016_federal"
    shutil.copytree(
        source_parquet,
        destination_parquet,
    ...
    for source in sorted((project_root / "data" / "manifests").glob("aec_2016*")):
        if source.is_file():
...
def _verify_packaged_2016_artifacts(project_root: Path) -> None:
    manifest_root = project_root / "data" / "manifests"
    source_manifest = json.loads(
        (manifest_root / "aec_2016_sources.json").read_text(encoding="utf-8")
    )
    if (
        source_manifest.get("election_id") != ELECTION_2016
        or source_manifest.get("source_count") != 46
        or len(source_manifest.get("sources") or []) != 46
    ):
        raise HistoricalReleaseError("The governed 2016 source manifest is incomplete.")
    _verified_manifest_files(
        project_root,
        source_manifest,
        source_manifest["sources"],
        label="AEC 2016 source",
    )

    formal_manifest = json.loads(
        (manifest_root / "aec_2016_formal_preferences.json").read_text(
            encoding="utf-8"
        )
    )
    expected_formal = {
        "state_count": 8,
        "ballot_count": 13_838_900,
        "preference_count": 93_842_251,
        "file_count": 53,
    }
    if {key: formal_manifest.get(key) for key in expected_formal} != expected_formal:
        raise HistoricalReleaseError(
            "The packaged 2016 formal-preference manifest does not reconcile."
        )
    _verified_manifest_files(
        project_root,
        formal_manifest,
        formal_manifest.get("files") or [],
        label="2016 formal-preference Parquet",
    )

    fact_manifest = json.loads(
        (manifest_root / "aec_2016_parquet.json").read_text(encoding="utf-8")
    )
    partitions = fact_manifest.get("partitions") or []
    if fact_manifest.get("partition_count") != len(partitions) or len(partitions) != 30:
        raise HistoricalReleaseError("The packaged 2016 fact-Parquet manifest is incomplete.")
    _verified_manifest_files(
        project_root,
        fact_manifest,
        partitions,
        label="2016 fact Parquet",
    )
    ...
    try:
        ...
        connection.execute(
            """INSERT OR IGNORE INTO control.database_release VALUES
               (?, ?, 'validated', ?, ?, 'Politica Stage 14.4 updater', ?)""",
            [
                release_id,
                schema_version,
                now,
                now,
                (
                    "Added the complete governed 2016 federal election alongside the "
                    "unchanged 2025, 2022 and 2019 elections, with election-specific "
                    "publication feeds."
                ),
            ],
        )
...
def _stage14_4_checks(
    ...
    expected_2016 = {
        ...
        "outcomes": 226,
        "count_rounds": 4956,
        "count_candidate_totals": 458285,
        "ballot_datasets": 8,
        "formal_ballots": 13838900,
        "source_revisions": 46,
    }
    ...
    checks = {
        ...
        "2016_counts": (observed_2016, expected_2016),
    ...
    return {
        ...
        "stage": "stage_14_4_2016_multi_election_release",
        "checks": len(checks),
        "failures": failures,
...
def _existing_publication(
    ...
    publication = verify_historical_release_bundle(
        ...
        release_root,
        release_id,
        stage_manifest_name="stage_14_4_release.json",
        election_id=ELECTION_2016,
        election_validation_key="election_2016",
        expected_active_elections=4,
...
def install_2016_release(settings: AppSettings | None = None) -> dict:
    settings = settings or AppSettings.from_environment()
    service = JobService(settings)
    project_root = settings.project_root.resolve()
    delta_manifest_path = project_root / "data" / "manifests" / "aec_2016_delta_tables.json"
    import_report_path = project_root / "dist" / "stage_14_4_2016_import_report.json"
    if not delta_manifest_path.is_file() or not import_report_path.is_file():
        raise HistoricalReleaseError("The prevalidated Stage 14.4 import assets are incomplete.")
    ...
    if recorded_manifest_sha256 != observed_manifest_sha256:
        raise HistoricalReleaseError("The 2016 delta-table manifest failed checksum verification.")
    import_report = json.loads(import_report_path.read_text(encoding="utf-8"))
    if import_report.get("status") != "PASS":
        raise HistoricalReleaseError("The packaged 2016 import report is not passing.")
    _verify_packaged_2016_artifacts(project_root)

    with FileLock(str(settings.releases_root / ".stage14_4.lock")):
        ...
        try:
            ...
            if installed:
                return {
                    ...
                    "election_2016": _election_counts(base_connection, ELECTION_2016),
                }
            ...
            if not base_2019_counts["contests"] or not base_2019_sources:
                raise HistoricalReleaseError(
                    "Stage 14.4 requires the successfully installed Stage 14.3 2019 release."
                )
        finally:
        ...
        job_id = f"stage14_4_2016_ingestion_{token}"
        release_id = f"release_1_6_0_2016_{token}"
        release_root = settings.releases_root / f"politica-stage14-4-{token}"
        temporary = settings.releases_root / (
            f".politica-stage14-4-{token}.tmp-{uuid.uuid4().hex}"
        )
        ...
        try:
            ...
            _copy_2016_artifacts(project_root, temporary)
            ...
            try:
                ...
                stage_validation = _stage14_4_checks(
                    candidate,
                    base_2025_counts,
                    base_2025_sources,
                    base_2022_counts,
            ...
            verify_historical_candidate_database(
                ...
                temporary,
                election_id=ELECTION_2016,
                expected_counts=stage_validation["election_2016"],
                expected_active_elections=4,
                election_counts=_election_counts,
                error_type=HistoricalReleaseError,
            )
            if stage_validation["status"] != "PASS":
                raise HistoricalReleaseError(
                    "The merged multi-election release failed Stage 14.4 reconciliation: "
                    + json.dumps(stage_validation["failures"], ensure_ascii=False)
                )
            if _sha256_file(base_database) != base["database_sha256"]:
                raise HistoricalReleaseError(
                    "The immutable 2025, 2022 and 2019 base release changed during installation."
                )
            ...
            manifest_path = temporary / "data" / "manifests" / "stage_14_4_release.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Install the prevalidated Stage 14.4 2016 multi-election release."
    )
    parser.parse_args()
    print(json.dumps(install_2016_release(), indent=2))
