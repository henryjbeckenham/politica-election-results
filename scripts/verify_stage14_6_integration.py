#!/usr/bin/env python3
"""Verify a clean Stage 3 -> 14.2 -> 14.3 -> 14.4 -> 14.5 -> 14.6 upgrade."""
...
def overlay_application(project: Path) -> None:
    """Install the Stage 14.6 application payload without touching project data."""
...
def copy_stage14_3_assets(
    core_zip: Path,
    data_zips: list[Path],
    project: Path,
) -> None:
    """Reconstruct the exact multipart Stage 14.3 installer payload."""

    with tempfile.TemporaryDirectory(prefix="politica-stage14-3-assets-") as temporary:
        staging = Path(temporary)
        with zipfile.ZipFile(core_zip) as archive:
            archive.extractall(staging / "core")
        payloads = list((staging / "core").glob("*/payload"))
        if len(payloads) != 1:
            raise RuntimeError("The Stage 14.3 core archive has an unexpected layout.")
        payload = payloads[0]
        shutil.copytree(
            payload / "data" / "stage14_3",
            project / "data" / "stage14_3",
            dirs_exist_ok=True,
        )
        manifest_root = project / "data" / "manifests"
        manifest_root.mkdir(parents=True, exist_ok=True)
        for source in (payload / "data" / "manifests").glob("aec_2019*"):
            shutil.copy2(source, manifest_root / source.name)
        (project / "dist").mkdir(parents=True, exist_ok=True)
        shutil.copy2(
            payload / "dist" / "stage_14_3_2019_import_report.json",
            project / "dist" / "stage_14_3_2019_import_report.json",
        )
        for index, data_zip in enumerate(data_zips):
            with zipfile.ZipFile(data_zip) as archive:
                archive.extractall(staging / f"data-{index}")
            asset_root = staging / f"data-{index}" / "stage14_3_assets" / "data"
            if not asset_root.is_dir():
                raise RuntimeError(f"The Stage 14.3 data archive is malformed: {data_zip.name}")
            shutil.copytree(asset_root, project / "data", dirs_exist_ok=True)
...
def copy_stage14_5_assets(project: Path) -> None:
    """Copy only manifest-governed 2013 assets into the isolated project."""

    shutil.copytree(
        PROJECT_ROOT / "data" / "stage14_5" / "tables",
        project / "data" / "stage14_5" / "tables",
        dirs_exist_ok=True,
    )
    shutil.copytree(
        PROJECT_ROOT / "data" / "parquet" / "aec_2013",
        project / "data" / "parquet" / "aec_2013",
        dirs_exist_ok=True,
    )
    source_manifest = json.loads(
    ...
    manifest_root = project / "data" / "manifests"
    manifest_root.mkdir(parents=True, exist_ok=True)
    for source in (PROJECT_ROOT / "data" / "manifests").glob("aec_2013*"):
        shutil.copy2(source, manifest_root / source.name)
    (project / "dist").mkdir(parents=True, exist_ok=True)
    shutil.copy2(
        PROJECT_ROOT / "dist" / "stage_14_5_2013_import_report.json",
        project / "dist" / "stage_14_5_2013_import_report.json",
    )


def copy_stage14_6_assets(project: Path) -> None:
    """Copy only manifest-governed 2010 assets into the isolated project."""

    shutil.copytree(
        PROJECT_ROOT / "data" / "stage14_6" / "tables",
        project / "data" / "stage14_6" / "tables",
        dirs_exist_ok=True,
    )
    shutil.copytree(
        PROJECT_ROOT / "data" / "parquet" / "aec_2010",
        project / "data" / "parquet" / "aec_2010",
        dirs_exist_ok=True,
    )
    source_manifest = json.loads(
    ...
    manifest_root = project / "data" / "manifests"
    manifest_root.mkdir(parents=True, exist_ok=True)
    for source in (PROJECT_ROOT / "data" / "manifests").glob("aec_2010*"):
        shutil.copy2(source, manifest_root / source.name)
    (project / "dist").mkdir(parents=True, exist_ok=True)
    shutil.copy2(
        PROJECT_ROOT / "dist" / "stage_14_6_2010_import_report.json",
        project / "dist" / "stage_14_6_2010_import_report.json",
    )
...
def verify(
    ...
    stage14_3_core: Path,
    stage14_3_data: list[Path],
    work_root: Path | None = None,
) -> dict:
    workspace = (
        nullcontext(str(work_root))
        if work_root is not None
        else tempfile.TemporaryDirectory(prefix="politica-stage14-6-integration-")
    )
    with workspace as temporary:
        root = Path(temporary)
        if work_root is not None and root.exists() and any(root.iterdir()):
            raise RuntimeError(
                "The retained integration work root must be empty for a clean verification."
            )
        ...
        copy_stage14_3_assets(stage14_3_core, stage14_3_data, project)
        copy_stage14_4_assets(project)
        copy_stage14_5_assets(project)
        copy_stage14_6_assets(project)
        ...
        stage14_4_database = Path(stage14_4["database_path"])
        if not stage14_4_database.is_absolute():
            stage14_4_database = project / stage14_4_database
        ...
        if sha256(base_database) != base_sha256:
            raise RuntimeError("The immutable 2025 baseline changed during Stage 14.5.")
        if sha256(stage14_2_database) != stage14_2_sha256:
            raise RuntimeError("The immutable 2025 + 2022 release changed during Stage 14.5.")
        if sha256(stage14_3_database) != stage14_3_sha256:
            raise RuntimeError(
                "The immutable 2025 + 2022 + 2019 release changed during Stage 14.5."
            )
        if sha256(stage14_4_database) != stage14_4_sha256:
            raise RuntimeError(
                "The immutable 2025 + 2022 + 2019 + 2016 release changed during Stage 14.5."
            )
        stage14_5_database = Path(stage14_5["database_path"])
        if not stage14_5_database.is_absolute():
            stage14_5_database = project / stage14_5_database
        ...
        preserved = (
            ...
            (stage14_5_database, stage14_5_sha256, "2025 + 2022 + 2019 + 2016 + 2013 release"),
        )
        for database, expected_sha256, label in preserved:
            if sha256(database) != expected_sha256:
...
def main() -> None:
    ...
    parser.add_argument("--stage14-3-core", type=Path, required=True)
    parser.add_argument("--stage14-3-data", type=Path, nargs=4, required=True)
    parser.add_argument(
        "--work-root",
        type=Path,
