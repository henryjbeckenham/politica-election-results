#!/usr/bin/env python3
"""Create the deterministic multipart Stage 14.3 Mac update."""
...
OUTPUT = ROOT / "dist" / "stage14_3_release"
CORE_NAME = "Politica_Stage14_3_v1.5.0_Update"
ASSET_ROOT = "stage14_3_assets"
DATA_PACKAGES = {
    "Politica_Stage14_3_v1.5.0_Data_1_NSW.zip": {"NSW"},
    "Politica_Stage14_3_v1.5.0_Data_2_VIC_SA.zip": {"VIC", "SA"},
    "Politica_Stage14_3_v1.5.0_Data_3_QLD_WA.zip": {"QLD", "WA"},
    "Politica_Stage14_3_v1.5.0_Data_4_ACT_NT_TAS_and_counts.zip": {
        "ACT",
        "NT",
        "TAS",
    },
...
def asset_files() -> dict[str, list[tuple[Path, str]]]:
    ...
    formal_root = ROOT / "data" / "parquet" / "aec_2019" / "formal_preferences"
    ...
    for path in sorted((ROOT / "data" / "parquet" / "aec_2019" / "facts").rglob("*.parquet")):
        grouped[first_package].append(
            (path, f"{ASSET_ROOT}/{path.relative_to(ROOT).as_posix()}")
        )

    source_manifest = json.loads(
        (ROOT / "data" / "manifests" / "aec_2019_sources.json").read_text(
            encoding="utf-8"
        )
    )
    for row in source_manifest["sources"]:
        ...
        state = None
        name = path.name
        if name.startswith("aec-senate-formalpreferences-24310-"):
            state = name.removeprefix(
                "aec-senate-formalpreferences-24310-"
            ).removesuffix(".zip")
...
def copy_payload(destination: Path) -> None:
    ...
    for path in sorted((ROOT / "data" / "manifests").glob("aec_2019*")):
    ...
    for filename in (
        "stage_14_3_2019_build_manifest.json",
        "stage_14_3_2019_import_report.json",
        "stage_14_3_build_manifest.json",
        "stage_14_3_integration_report.json",
        "stage_14_3_test_report.json",
        "stage_14_3_unittest.log",
        "stage_14_3_stage8_rerun.log",
        "stage_14_3_stage9_rerun.log",
        "stage_14_3_browser.log",
    ):
...
def build() -> dict:
    required = (
        ROOT / "dist" / "stage_14_3_integration_report.json",
        ROOT / "dist" / "stage_14_3_test_report.json",
        ROOT / "src" / "politica_erd" / "app" / "results" / "index.html",
    )
    for path in required:
        if not path.is_file():
            raise RuntimeError(f"Stage 14.3 packaging prerequisite is missing: {path}")
    ...
    build_manifest = {
        ...
        "full_regression_report": "dist/stage_14_3_test_report.json",
        "clean_install_report": "dist/stage_14_3_integration_report.json",
    }
    (ROOT / "dist" / "stage_14_3_build_manifest.json").write_text(
        json.dumps(build_manifest, indent=2) + "\n", encoding="utf-8"
    )
    (ROOT / "data" / "manifests" / "stage14_3_asset_packages.json").write_text(
        json.dumps({"packages": package_rows}, indent=2) + "\n", encoding="utf-8"
    )

    with tempfile.TemporaryDirectory(prefix="politica-stage14-3-package-") as temporary:
        core = Path(temporary) / CORE_NAME
        core.mkdir()
        copy_payload(core)
        installer = (ROOT / "install_stage14_3.command").read_text(encoding="utf-8")
        replacements = {
        ...
        installer_path = core / "install_stage14_3.command"
        installer_path.write_text(installer, encoding="utf-8")
        installer_path.chmod(0o755)
        shutil.copy2(ROOT / "packaging" / "READ_ME_STAGE14_3.md", core / "READ_ME_FIRST.md")
        core_files = [
