#!/usr/bin/env python3
"""Build the governed browser geometry for the 2010 House map."""
...
def division_numbers(source: Path) -> dict[str, int]:
    ...
    with source.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        for row in rows:
            ...
            result[key] = observed
    if len(result) != 150:
        raise RuntimeError("The 2010 candidate source must identify 150 divisions")
    return result


def build(source: Path, candidates: Path, output: Path, tolerance: float) -> dict:
    identifiers = division_numbers(candidates)
    with tempfile.TemporaryDirectory(prefix="politica-boundaries-2010-") as temporary:
        with zipfile.ZipFile(source) as archive:
            archive.extractall(temporary)
        reader = shapefile.Reader(str(Path(temporary) / "COM_ELB_2010_region.shp"))
        features = []
        for item in reader.iterShapeRecords():
            ...
            try:
                ...
                raise RuntimeError(
                    f"The AEC boundary division is absent from the 2010 result: {electorate}"
                ) from exc
    ...
    if len(features) != 150 or len(names) != len(set(names)):
        raise RuntimeError("The 2010 boundary source must contain 150 unique divisions")
    if set(names) != set(identifiers):
        missing = sorted(set(identifiers) - set(names))
        extra = sorted(set(names) - set(identifiers))
        raise RuntimeError(
            f"The 2010 result and boundary divisions differ; missing={missing}, extra={extra}"
        )
