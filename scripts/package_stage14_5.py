#!/usr/bin/env python3
"""Create the deterministic multipart Stage 14.5 Mac update."""
...
OUTPUT = ROOT / "dist" / "stage14_5_release"
CORE_NAME = "Politica_Stage14_5_v1.7.0_Update"
ASSET_ROOT = "stage14_5_assets"
DATA_PACKAGES = {
    "Politica_Stage14_5_v1.7.0_Data_1_NSW.zip": {"NSW"},
    "Politica_Stage14_5_v1.7.0_Data_2_VIC_SA.zip": {"VIC", "SA"},
    "Politica_Stage14_5_v1.7.0_Data_3_QLD_WA.zip": {"QLD", "WA"},
    "Politica_Stage14_5_v1.7.0_Data_4_ACT_NT_TAS_and_counts.zip": {
        "ACT",
        "NT",
        "TAS",
    },
...
def _btl_state(filename: str) -> str | None:
    prefix = "SenateStateBtlDownload-17496-"
    if filename.startswith(prefix) and filename.endswith(".zip"):
        state = filename.removeprefix(prefix).removesuffix(".zip").upper()
        if state in {item for states in DATA_PACKAGES.values() for item in states}:
            return state
    return None
...
def asset_files() -> dict[str, list[tuple[Path, str]]]:
    ...
    formal_root = ROOT / "data" / "parquet" / "aec_2013" / "formal_preferences"
    ...
    facts = ROOT / "data" / "parquet" / "aec_2013" / "facts"
    for path in sorted(facts.rglob("*.parquet")):
        grouped[first_package].append(
            (path, f"{ASSET_ROOT}/{path.relative_to(ROOT).as_posix()}")
        )

    source_manifest = json.loads(
        (ROOT / "data" / "manifests" / "aec_2013_sources.json").read_text(
            encoding="utf-8"
        )
    )
    for row in source_manifest["sources"]:
        path = ROOT / row["path"]
        state = _btl_state(path.name)
    ...
    table_files = sorted(
        (ROOT / "data" / "stage14_5" / "tables").rglob("*.parquet"),
    ...
    for path in table_files:
        package = min(grouped, key=lambda name: (package_sizes[name], name))
        grouped[package].append(
            (path, f"{ASSET_ROOT}/{path.relative_to(ROOT).as_posix()}")
        )
...
def copy_payload(destination: Path) -> None:
    ...
    for directory in (
        ...
        shutil.copytree(
            ...
            ignore=shutil.ignore_patterns(
                "node_modules", ".cache", "*.pyc", "__pycache__"
            ),
        )
    for filename in (
        ".gitignore",
    ...
    for path in sorted((ROOT / "data" / "manifests").glob("aec_2013*")):
        shutil.copy2(path, manifest_target / path.name)
    dist_target = payload / "dist"
    dist_target.mkdir(parents=True)
    for filename in (
        "stage_14_5_2013_build_manifest.json",
        "stage_14_5_2013_import_report.json",
        "stage_14_5_build_manifest.json",
        "stage_14_5_integration_report.json",
        "stage_14_5_test_report.json",
        "stage_14_5_unittest.log",
        "stage_14_5_browser.log",
    ):
...
def build() -> dict:
    required = (
        ROOT / "dist" / "stage_14_5_integration_report.json",
        ROOT / "dist" / "stage_14_5_test_report.json",
        ROOT / "src" / "politica_erd" / "app" / "results" / "index.html",
    )
    for path in required:
        if not path.is_file():
            raise RuntimeError(f"Stage 14.5 packaging prerequisite is missing: {path}")
        ...
        package_rows.append(
            {
                ...
                "uncompressed_size_bytes": sum(
                    path.stat().st_size for path, _ in files
                ),
            }
        )
    ...
    build_manifest = {
        ...
        "full_regression_report": "dist/stage_14_5_test_report.json",
        "clean_install_report": "dist/stage_14_5_integration_report.json",
    }
    (ROOT / "dist" / "stage_14_5_build_manifest.json").write_text(
        json.dumps(build_manifest, indent=2) + "\n", encoding="utf-8"
    )
    (ROOT / "data" / "manifests" / "stage14_5_asset_packages.json").write_text(
        json.dumps({"packages": package_rows}, indent=2) + "\n", encoding="utf-8"
    )

    with tempfile.TemporaryDirectory(prefix="politica-stage14-5-package-") as temporary:
        core = Path(temporary) / CORE_NAME
        core.mkdir()
        copy_payload(core)
        installer = (ROOT / "install_stage14_5.command").read_text(
            encoding="utf-8"
        )
        replacements = {
        ...
        installer_path = core / "install_stage14_5.command"
        installer_path.write_text(installer, encoding="utf-8")
        installer_path.chmod(0o755)
        shutil.copy2(
            ROOT / "packaging" / "READ_ME_STAGE14_5.md",
            core / "READ_ME_FIRST.md",
        )
        core_files = [
