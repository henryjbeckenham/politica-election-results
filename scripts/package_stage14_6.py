#!/usr/bin/env python3
"""Create the deterministic multipart Stage 14.6 Mac update."""
...
OUTPUT = ROOT / "dist" / "stage14_6_release"
CORE_NAME = "Politica_Stage14_6_v1.8.0_Update"
ASSET_ROOT = "stage14_6_assets"
DATA_PACKAGES = {
    "Politica_Stage14_6_v1.8.0_Data_1_NSW.zip": {"NSW"},
    "Politica_Stage14_6_v1.8.0_Data_2_VIC_SA.zip": {"VIC", "SA"},
    "Politica_Stage14_6_v1.8.0_Data_3_QLD_WA.zip": {"QLD", "WA"},
    "Politica_Stage14_6_v1.8.0_Data_4_ACT_NT_TAS_and_counts.zip": {
        "ACT",
        "NT",
        "TAS",
    },
...
def _btl_state(filename: str) -> str | None:
    prefix = "SenateStateBtlDownload-15508-"
...
def asset_files() -> dict[str, list[tuple[Path, str]]]:
    ...
    formal_root = ROOT / "data" / "parquet" / "aec_2010" / "formal_preferences"
    ...
    facts = ROOT / "data" / "parquet" / "aec_2010" / "facts"
    ...
    source_manifest = json.loads(
        (ROOT / "data" / "manifests" / "aec_2010_sources.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    table_files = sorted(
        (ROOT / "data" / "stage14_6" / "tables").rglob("*.parquet"),
...
def copy_payload(destination: Path) -> None:
    ...
    for path in sorted((ROOT / "data" / "manifests").glob("aec_2010*")):
        shutil.copy2(path, manifest_target / path.name)
    dist_target = payload / "dist"
    dist_target.mkdir(parents=True)
    for filename in (
        "stage_14_6_2010_build_manifest.json",
        "stage_14_6_2010_import_report.json",
        "stage_14_6_build_manifest.json",
        "stage_14_6_integration_report.json",
        "stage_14_6_test_report.json",
        "stage_14_6_unittest.log",
        "stage_14_6_browser.log",
    ):
...
def build() -> dict:
    required = (
        ROOT / "dist" / "stage_14_6_integration_report.json",
        ROOT / "dist" / "stage_14_6_test_report.json",
        ROOT / "src" / "politica_erd" / "app" / "results" / "index.html",
    )
    for path in required:
        if not path.is_file():
            raise RuntimeError(f"Stage 14.6 packaging prerequisite is missing: {path}")
    ...
    build_manifest = {
        ...
        "full_regression_report": "dist/stage_14_6_test_report.json",
        "clean_install_report": "dist/stage_14_6_integration_report.json",
    }
    (ROOT / "dist" / "stage_14_6_build_manifest.json").write_text(
        json.dumps(build_manifest, indent=2) + "\n", encoding="utf-8"
    )
    (ROOT / "data" / "manifests" / "stage14_6_asset_packages.json").write_text(
        json.dumps({"packages": package_rows}, indent=2) + "\n", encoding="utf-8"
    )

    with tempfile.TemporaryDirectory(prefix="politica-stage14-6-package-") as temporary:
        core = Path(temporary) / CORE_NAME
        core.mkdir()
        copy_payload(core)
        installer = (ROOT / "install_stage14_6.command").read_text(
            encoding="utf-8"
        )
        replacements = {
        ...
        installer_path = core / "install_stage14_6.command"
        installer_path.write_text(installer, encoding="utf-8")
        installer_path.chmod(0o755)
        shutil.copy2(
            ROOT / "packaging" / "READ_ME_STAGE14_6.md",
